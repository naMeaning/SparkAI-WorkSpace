...
func resolveTaskDurationSeconds(req TaskSubmitReq) (int, error) {
	if strings.TrimSpace(req.Seconds) != "" {
		seconds, err := strconv.Atoi(req.Seconds)
		if err != nil {
			return 0, fmt.Errorf("seconds is invalid")
		}
		if seconds != 0 {
			return seconds, nil
		}
...
func validateTaskDurationSeconds(seconds int) *dto.TaskError {
	if seconds < 0 || seconds > MaxTaskDurationSeconds {
		return createTaskError(
			fmt.Errorf("seconds must be between 0 and %d", MaxTaskDurationSeconds),
			"invalid_seconds",
...
func validateTaskDurationValue(value any) *dto.TaskError {
	var seconds int
	switch v := value.(type) {
	case nil:
		return nil
	case int:
		seconds = v
	case int64:
		if v > int64(MaxTaskDurationSeconds) || v < 0 {
			return validateTaskDurationSeconds(MaxTaskDurationSeconds + 1)
		}
		seconds = int(v)
	case float64:
		if v > float64(MaxTaskDurationSeconds) || v < 0 {
			return validateTaskDurationSeconds(MaxTaskDurationSeconds + 1)
		}
		seconds = int(v)
	case string:
	...
	return validateTaskDurationSeconds(seconds)
}

func validateTaskMetadataDuration(metadata map[string]interface{}) *dto.TaskError {
	if metadata == nil {
		return nil
	}
	for _, key := range []string{"durationSeconds", "duration_seconds"} {
		if value, ok := metadata[key]; ok {
			if taskErr := validateTaskDurationValue(value); taskErr != nil {
				return taskErr
			}
		}
	}
...
func ValidateMultipartDirect(c *gin.Context, info *RelayInfo) *dto.TaskError {
	...
	size = req.Size
	var err error
	seconds, err = resolveTaskDurationSeconds(req)
	if err != nil {
		return createTaskError(err, "invalid_seconds", http.StatusBadRequest, true)
	}
	if taskErr := validateTaskDurationSeconds(seconds); taskErr != nil {
		return taskErr
	}
	if taskErr := validateTaskMetadataDuration(req.Metadata); taskErr != nil {
		return taskErr
	}
