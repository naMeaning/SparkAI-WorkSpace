...
输入 `.env` 中的 `SPARKAI_EXTENSION_ADMIN_TOKEN` 即可创建和管理兑换码。可设置每个兑换码的设备使用次数、激活后的授权天数、兑换截止时间；列表会显示已用次数，并可禁用兑换码。禁用会同时撤销该码已经发出的设备 License。管理员 Token 不要放进 URL、截图或普通聊天；刷新或退出页面后需要重新输入。
