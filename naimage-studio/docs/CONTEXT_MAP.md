...
Electron Main: electron-main.cjs
  ...
  ├─ desktop/ipc/register-desktop-ipc.cjs：141 个 invoke handler（138 preload + 3 internal）/9 receive/2 send channel 的唯一注册顺序
...
| `desktop/ipc/register-desktop-ipc.cjs`, `desktop/ipc/*-ipc.cjs` | Settings → GlassBackground → RequirementLibrary → CommerceTemplate → CommerceCatalog → CommerceExport → SocialExport → Scientific → Plugin → Automation → Updater → Session → Agent → Window → Debug → Project → ImageCollection → Asset → Server → VideoTask 的固定注册顺序和各域 handler | 桌面服务实现、React 状态、跨域业务复制；依赖必须由 Main 显式注入 | `registerDesktopIpc`, `registerSettingsIpc`, `registerGlassBackgroundIpc`, `registerRequirementLibraryIpc`, `registerCommerceTemplateIpc`, `registerCommerceCatalogIpc`, `registerCommerceExportIpc`, `registerSocialExportIpc`, `registerScientificIpc`, `registerPluginIpc`, `registerAutomationIpc`, `registerAgentIpc`, `registerImageCollectionIpc`, `registerAssetIpc`, `registerServerIpc`, `registerVideoTaskIpc` | `test:ipc-registration`, `test:lifecycle`, `aidebug:gui` |
...
| `desktop/image-collection-export-service.cjs`, `desktop/ipc/image-collection-ipc.cjs` | Main-only 图片组 session 复核、PNG/JPEG/WebP/AVIF/TIFF 统计预检与内容令牌、受管资产校验、staging/原子发布、manifest 与已导出目录解析；preload 仅暴露预检/导出/打开文件夹的有界 payload | 不接受 Renderer 绝对路径、不写入项目外目录、不复用漂移令牌、不伪造未导出成功；所有文件操作留在 Main | `test:image-collection-export`, `test:ipc-registration`, `test:automation-service`, `aidebug:image-collection` |
...
| `window.naimageConfig` | 设置、显式项目/session、旧数据迁移预检/执行/清理、图片组预检/导出/目录、图片另存、受管工作区背景、Project Graph 只读导入、社媒发布包预检/导出、窗口控制 | 本地文件、原生 Open/Save 授权、项目目录、迁移回执、BrowserWindow、原子保存、受限 `.prg` 解析、背景 WebP 转码与社媒原生目录授权 |
...
| 图片导入/缩略图 | `image-import.cjs`, `thumbnail-cache.cjs`, `image-thumbnail-worker.cjs`, `electron-main.cjs`, `src/core.ts`, `src/main.tsx` | 资产身份、路径限制、容器；项目私有 WebP 缓存按来源指纹和 256/512/1024 桶复用，每个冷 miss 由 Main 最多 2 并发地 `fork()` 一次性 Sharp 子进程并排队，默认只保留 96 个变体；单张生成成果可在画布直接使用原图，查看器主图始终读取原图。性能诊断必须区分冷生成队列、缓存命中与 Renderer 长任务 | `test:image-import`, `test:thumbnail-cache`, `aidebug:performance`, AIDebug import |
...
| `<project>/exports/image-groups/` | 单组/多组格式化图片与 `image-group.json` | Electron Main；预检令牌、staging、备份和原子发布 |
...
| 旧 AppData 项目/全局 Session、项目 Agent 状态、空间预检、SHA-256、原子迁移、回滚与二次确认清理 | `corepack pnpm run test:project-data-migration`（41 cases）、`corepack pnpm run test:project-root-policy`、`corepack pnpm run test:ipc-registration`；真实隔离 GUI 为 `corepack pnpm run aidebug:isolation` |
...
| 图片组命名、受管替换/瑕疵组、PNG/JPEG/WebP/AVIF/TIFF 预检/令牌/统计批量导出与目录入口 | `corepack pnpm run test:image-collection-mutation`（4 cases）、`corepack pnpm run test:image-collection-export`（受管资产、格式转换、内容漂移、manifest、排序、重命名、回滚与目录解析）、`corepack pnpm run test:automation-service`、`corepack pnpm run test:ipc-registration`、`corepack pnpm run typecheck`；真实隔离 Electron 为 `corepack pnpm run aidebug:image-collection`，权威报告 `.diagnostics/electron/aidebug-2026-08-14T03-54-29-294Z/report.json` 为 9 scenes、0 failures |
...
| IPC 注册顺序/preload 对称性 | `corepack pnpm run test:ipc-registration`；当前合同为 141 invoke handlers / 138 preload invokes / 3 internal Agent invokes / 9 receives / 2 sends |
...
- `src/main.tsx`、`electron-main.cjs`、`agent-runtime.cjs` 和 `src/core.ts` 仍较大，但已分别建立 renderer surface、desktop domain、runtime domain 与纯数据模块边界；Main 的 141 个 invoke handler（138 个 preload invoke + 3 个内部 Agent invoke）、9 个进度/命令接收和 2 个 preload send channel 已由 `desktop/ipc/*` 独立拥有，`agent-runtime.cjs` 的 memory store、tool schema、Responses/Chat parser 与图片批次调度也已有独立 owner，后续继续沿现有边界拆，不要重新内联。
