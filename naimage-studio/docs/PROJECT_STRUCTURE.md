...
本仓库只维护 Windows Electron 客户端与本地单 Agent runtime。用户原生 New API、账户渠道与下载站由外部部署维护，`sparkai-extension` 只提供 License 与图片任务扩展，不复制到这里。符号级所有权、进程调用链与测试影响见 [CONTEXT_MAP.md](./CONTEXT_MAP.md)。
