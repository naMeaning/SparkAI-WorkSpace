...
export function useFloatingDialogInteractions() {
  useEffect(() => {
    ...
    const resizeMetrics = __NAIMAGE_AIDEBUG__
      ? (window.__naimageResizeObserverMetrics ??= {
          created: 0,
          disconnected: 0,
          callbacks: 0,
          observed: 0,
    ...
    function beginFloatingDrag(event: PointerEvent) {
      ...
      if (__NAIMAGE_AIDEBUG__) {
        window.__naimageLastFloatingDrag = {
          phase: "down",
          source: "pointer",
          button: event.button,
      ...
      if (event.button !== 0) {
        if (__NAIMAGE_AIDEBUG__) window.__naimageLastFloatingDrag = { ...window.__naimageLastFloatingDrag, phase: "ignored", reason: "non-primary-button" };
        return;
      }
      const target = rawTarget instanceof HTMLElement ? rawTarget : rawTarget?.parentElement ?? null;
      if (!target) {
        if (__NAIMAGE_AIDEBUG__) window.__naimageLastFloatingDrag = { ...window.__naimageLastFloatingDrag, phase: "ignored", reason: "target-missing" };
        return;
      }
      if (target.closest(FLOATING_DIALOG_INTERACTIVE_SELECTOR)) {
        if (__NAIMAGE_AIDEBUG__) window.__naimageLastFloatingDrag = { ...window.__naimageLastFloatingDrag, phase: "ignored", reason: "interactive-target" };
        return;
      }
      const handle = target.closest(FLOATING_DIALOG_HANDLE_SELECTOR) as HTMLElement | null;
      if (!handle) {
        if (__NAIMAGE_AIDEBUG__) window.__naimageLastFloatingDrag = { ...window.__naimageLastFloatingDrag, phase: "ignored", reason: "handle-missing" };
        return;
      }
      ...
      function end() {
        ...
        if (__NAIMAGE_AIDEBUG__) window.__naimageLastFloatingDrag = { ...window.__naimageLastFloatingDrag, phase: "end", source: "pointer", left: activeDialog.style.left, top: activeDialog.style.top };
