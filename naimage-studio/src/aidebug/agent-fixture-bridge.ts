...
declare global {
  interface Window {
    __naimageDebugApplyAgentActions?: (actions: AgentRuntimeAction[]) => boolean;
    __naimageDebugSeedAgentMessages?: (payload?: AgentFixtureSeedPayload | null) => boolean;
  }
}
...
export function installAgentFixtureBridge({
  ...
  window.__naimageDebugApplyAgentActions = (actions: AgentRuntimeAction[]) => {
    if (!Array.isArray(actions)) return false;
    applyActions(actions);
    return true;
  };
  window.__naimageDebugSeedAgentMessages = (payload) => {
  ...
  return () => {
    cancelStreamFlush();
    streamAccumulator.clear();
    delete window.__naimageDebugApplyAgentActions;
    delete window.__naimageDebugSeedAgentMessages;
  };
}
