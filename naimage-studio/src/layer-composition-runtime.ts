...
export async function prepareLayerCompositionAssets(composition: ImageLayerComposition, projectId: string): Promise<ImageLayerComposition> {
    ...
    const saveLayerDataUrl = async (dataUrl: string, stem: string, runId: string) => {
      if (!window.naimageConfig?.saveOutputImage) throw new Error("项目图片库不可用，无法保存独立 PNG 图层。");
    ...
    const refineSemanticLayers = window.naimageConfig?.refineSemanticLayers;
    ...
    if (previewDataUrl && backgroundPrepared && transparentPrepared.length) {
      ...
      const forcedResidualRatio = __NAIMAGE_AIDEBUG__
        ? Math.min(1, Math.max(0, Number((window as unknown as { __naimageDebugLayerCoverageFailure?: number }).__naimageDebugLayerCoverageFailure || 0)))
        : 0;
