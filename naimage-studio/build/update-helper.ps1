...
function Restore-Backup([string]$Backup, [string]$Target) {
  ...
  $failedTarget = "$Target.naimage-failed"
...
$backup = "$Target.naimage-backup"
$staging = "$Target.naimage-new"
$launched = $null
...
try {
  ...
  if (-not (Wait-ForExit -ProcessId $ParentPid -Seconds $ParentExitTimeoutSeconds)) {
    throw "naimage did not exit before the update timeout."
  }
  ...
  $launched = Start-Process -FilePath $AppExe -ArgumentList @("--updated", "--naimage-update-token=$Token") -PassThru
  ...
  $failureCleanup = @($staging, "$Target.naimage-failed", $HealthFile)
