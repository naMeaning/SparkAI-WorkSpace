...
try {
  ...
  const actionHook = testWindow.__naimageDebugApplyAgentActions!;
  const messageHook = testWindow.__naimageDebugSeedAgentMessages!;
  ...
  assert.equal(timers.size, 1);
  cleanup();
  assert.equal(testWindow.__naimageDebugApplyAgentActions, undefined);
  assert.equal(testWindow.__naimageDebugSeedAgentMessages, undefined);
  assert.equal(timers.size, 0);
} finally {
  if (originalWindow) {
