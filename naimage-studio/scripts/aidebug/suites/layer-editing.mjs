...
async function captureLayerStackSuiteProbe(client, targetId) {
  ...
  await waitForExpression(client, "Boolean(window.__naimageAIDebug?.runLayerStackSuite && window.__naimageDebugAgentState)", 10000);
  await evaluate(client, `window.__naimageAIDebug?.resetLayerDetachAudit?.()`);
  ...
  const suitePromise = evaluate(
    client,
    `window.__naimageAIDebug.runLayerStackSuite({ agentDriven: ${JSON.stringify(realAgent)}, liveImage: ${JSON.stringify(liveImage)} })`,
  ...
  if (!suite?.groupId) {
    ...
    const failedState = await evaluate(client, `window.__naimageDebugAgentState?.()`);
  ...
  const postSuiteLayerState = await evaluate(client, `window.__naimageAIDebug?.layerState?.()`);
  ...
  if (unexpectedlyDetached.length) {
    ...
    const recovery = await evaluate(
      client,
      `window.__naimageAIDebug?.recomposeLayer?.({ id: ${JSON.stringify(String(unexpectedlyDetached[0]?.id || suite.subjectId || suite.stackId || ""))} })`,
      30000
    );
    layerDriftAudit.recovery = recovery;
    try {
      await waitForExpression(client, `Boolean(window.__naimageDebugAgentState?.().nodes?.filter((item) => item.layerGroup?.id === ${JSON.stringify(String(suite.groupId))}).every((item) => item.layerGroup?.detached === false))`, 8000);
    } catch {
  ...
  suite.state = await evaluate(client, `window.__naimageDebugAgentState?.()`);
  ...
  try {
    ...
    const folderOk = Boolean(
      ...
      manifest?.format === "naimage-layer-export" && manifest?.version === 1 && Array.isArray(manifest?.layers) && manifest.layers.length === 6 &&
  ...
  const detachedMenu = await evaluate(client, `window.__naimageAIDebug?.openNodeMenu?.({ id: ${JSON.stringify(detachedNodeId)} })`, 10000);
  ...
  await waitForExpression(client, `Boolean(window.__naimageDebugAgentState?.().nodes?.filter((item) => item.layerGroup?.id === ${JSON.stringify(String(suite?.groupId || ""))}).every((item) => item.layerGroup?.detached === true))`, 5000);
  const detachedFit = await evaluate(client, `window.__naimageAIDebug?.fitCanvas?.()`, 10000);
  const detachedState = await evaluate(client, `window.__naimageDebugAgentState?.()`);
  ...
  const recomposedAction = await evaluate(
    client,
    `window.__naimageAIDebug?.recomposeLayer?.({ id: ${JSON.stringify(String(suite?.subjectId || suite?.stackId || ""))} })`,
    30000
  );
  await evaluate(client, `window.__naimageAIDebug?.fitCanvas?.()`, 10000);
  await waitForExpression(client, `Boolean(window.__naimageDebugAgentState?.().nodes?.filter((item) => item.layerGroup?.id === ${JSON.stringify(String(suite?.groupId || ""))}).every((item) => item.layerGroup?.detached === false))`, 5000);
  ...
  const editorSpaceProof = await evaluate(client, `window.__naimageDebugAgentState?.().nodeEditorUi?.spaceUse || null`);
  ...
  const compactEditorProof = await evaluate(client, `(() => {
    ...
    const spaceUse = window.__naimageDebugAgentState?.().nodeEditorUi?.spaceUse || null;
    const rowGaps = [];
...
async function captureCutoutSuiteProbe(client, targetId) {
  await setWindowSize(client, targetId, 1280, 820);
  await evaluate(client, `window.__naimageDebugOpenSurface?.("main")`);
  await waitForExpression(client, "Boolean(window.__naimageAIDebug?.runCutoutSuite && window.__naimageDebugAgentState)", 10000);
  ...
  const suite = await evaluate(
    client,
    `window.__naimageAIDebug.runCutoutSuite({ liveImage: ${JSON.stringify(liveImage)} })`,
...
async function captureRegionRedrawSuiteProbe(client, targetId) {
  await setWindowSize(client, targetId, 1280, 820);
  await evaluate(client, `window.__naimageDebugOpenSurface?.("main")`);
  await waitForExpression(client, "Boolean(window.__naimageAIDebug?.runRegionRedrawSuite && window.__naimageDebugAgentState)", 10000);
  ...
  const suite = await evaluate(
    client,
    `window.__naimageAIDebug.runRegionRedrawSuite({ liveImage: ${JSON.stringify(liveImage)} })`,
  ...
  if (suite?.sourceId) {
    await evaluate(
      client,
      `(async () => {
        await window.__naimageAIDebug.selectNode({ id: ${JSON.stringify(String(suite.sourceId))} });
        return window.__naimageAIDebug.runTool(${JSON.stringify({
          name: "image_gen",
    ...
    await waitForExpression(client, "Boolean(document.querySelector('.region-redraw-dialog') && window.__naimageDebugAgentState?.().regionRedrawReady)", 12000);
  }
