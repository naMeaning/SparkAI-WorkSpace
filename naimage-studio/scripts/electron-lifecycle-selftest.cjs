...
async function main() {
  ...
  const child = spawn(electronExecutable, [`--user-data-dir=${path.join(runDir, "user-data")}`, "electron-main.cjs"], {
    ...
    env: {
      ...process.env,
      NAIMAGE_LIFECYCLE_SELFTEST: "1",
      NAIMAGE_LOCAL_SERVER_ENTRY: path.join(repoRoot, "scripts", "lifecycle-server-fixture.cjs"),
      NAIMAGE_CONFIG_DIR: path.join(runDir, "config"),
