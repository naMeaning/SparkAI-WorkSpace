  ...
  for (let index = 1; index <= cycles; index += 1) {
    ...
    const exitCode = await new Promise((resolve) => {
      const child = spawn(process.execPath, args, {
        ...
        stdio: "inherit",
        shell: false,
        env: { ...process.env, NAIMAGE_AIDEBUG_CHILD: "1" }
      });
...
function spawnElectron() {
  ...
  return spawn(command, commandArgs, {
    ...
    env: {
      ...
      NAIMAGE_AIDEBUG: "1",
      NAIMAGE_AIDEBUG_LIVE_IMAGE: liveImage ? "1" : "0",
      NAIMAGE_AIDEBUG_REAL_AGENT: mockAgent ? "0" : "1",
      NAIMAGE_AIDEBUG_MOCK_AGENT: mockAgent ? "1" : "0",
      NAIMAGE_AIDEBUG_AGENT_MODE: mockAgent ? "mock" : "real",
      NAIMAGE_AIDEBUG_AUTH_SESSION: authGateSuiteOnly ? "1" : "0",
      NAIMAGE_AIDEBUG_IMAGE_FAULTS: imageRecoveryOnly ? "1" : (process.env.NAIMAGE_AIDEBUG_IMAGE_FAULTS || "0"),
      NAIMAGE_AIDEBUG_ASSERT_EXPLICIT_LAYER_HINT: layerStackOnly ? "1" : (process.env.NAIMAGE_AIDEBUG_ASSERT_EXPLICIT_LAYER_HINT || "0"),
      ...
      try {
        const runtime = await secondary.send("Runtime.evaluate", {
          expression: `(() => ({
            ...
            agentState: window.__naimageDebugAgentState?.() || null,
  ...
  while (Date.now() - startedAt < timeoutMs) {
    try {
      const probe = await evaluate(client, `(async () => {
        ...
        return {
          sentinel: window.__naimageAuthReloadSentinel || '',
          readyState: document.readyState,
...
async function setWindowSize(client, targetId, width, height) {
  ...
  try {
    const ipcResult = await evaluate(
      client,
      `(async () => await window.naimageConfig?.debugWindowBounds?.(${JSON.stringify({ width, height })}))()`,
      5000
    );
  ...
  return `new Promise((resolve) => {
    ...
    const waitForDebugHook = async () => {
      ...
      while (Date.now() < deadline) {
        if (typeof window.__naimageDebugOpenSurface === "function") return true;
        await delay(40);
      }
      return typeof window.__naimageDebugOpenSurface === "function";
    };
    ...
    const invokeSurface = async (name, selector = "") => {
      ...
      do {
        ...
        window.__naimageDebugOpenSurface?.(name);
        await delay(80);
...
function agentMessageFixtureExpression(scrollMode = "bottom") {
  ...
  return `new Promise((resolve) => {
    window.__naimageDebugOpenSurface?.("agent-timeline");
    const seeded = window.__naimageDebugSeedAgentMessages?.({ messages: ${JSON.stringify(messages)}, append: false });
...
function stopThenResendExpression(sentinel) {
  ...
  return `new Promise((resolve) => {
    ...
    window.setTimeout(() => {
      window.__naimageDebugOpenSurface?.("agent-running");
      window.setTimeout(() => {
  ...
  return `new Promise((resolve) => {
    ...
    const submitSupplement = () => {
      ...
      const timer = window.setInterval(() => {
        ...
        if (sendSupplement) {
          window.clearInterval(timer);
          window.__naimageBusySupplementButtonClicked = true;
    ...
    window.__naimageBusySupplementButtonClicked = false;
    window.__naimageDebugOpenSurface?.("main");
    window.setTimeout(() => {
      window.__naimageDebugOpenSurface?.("agent-running");
      window.setTimeout(() => {
...
function runningReferenceRemoveExpression() {
  return `new Promise((resolve) => {
    window.__naimageDebugOpenSurface?.("main");
    window.setTimeout(() => {
      window.__naimageDebugOpenSurface?.("agent-running-reference-limit");
      window.setTimeout(() => {
...
function mixedSelectedContinuationExpression(sequenceIds, options = {}) {
  ...
  return `new Promise(async (resolve) => {
    ...
    try {
      ...
      window.__naimageDebugOpenSurface?.("main");
      await sleep(260);
      const initialState = window.__naimageDebugAgentState?.();
      const beforeIds = new Set((initialState?.nodes || []).map((node) => node.id));
      const steps = [];
      for (const id of sequence) {
        const detail = await window.__naimageAIDebug?.selectNode?.({ id });
        await sleep(180);
        const state = window.__naimageDebugAgentState?.();
        steps.push({ id, ok: Boolean(detail?.ok && state?.selectedNodeId === id), selectedNodeId: state?.selectedNodeId || "", detail });
      }
      const selectedBeforeChat = window.__naimageDebugAgentState?.()?.selectedNodeId || "";
      const beforeChatState = window.__naimageDebugAgentState?.();
      const beforeChatIds = new Set((beforeChatState?.nodes || []).map((node) => node.id));
      const chat = await window.__naimageAIDebug?.chat?.(prompt, { skipAsk: true });
      await sleep(260);
      const afterState = window.__naimageDebugAgentState?.();
...
async function captureState(client, targetId, label, setupExpression, size, expectedState = {}, setupTimeoutMs = 30000, visualPolicy = "auto") {
  ...
  if (typeof expectedState?.selectedNodeId === "string" && expectedState.selectedNodeId) {
    try {
      await waitForExpression(
        client,
        `window.__naimageDebugAgentState?.().selectedNodeId === ${JSON.stringify(expectedState.selectedNodeId)}`,
        2400
      );
      await delay(160);
    } catch {
  ...
  if (expectedState?.imageNodeViewportOk) {
    try {
      await waitForExpression(client, "Boolean(window.__naimageDebugAgentState?.().imageNodeViewportOk)", 1800);
      await delay(160);
    } catch {
  ...
  const preFrameSnapshot = await evaluate(client, "window.__naimageDebugLayoutSnapshot ? window.__naimageDebugLayoutSnapshot() : null");
  ...
  const postFrameSnapshot = await evaluate(client, "window.__naimageDebugLayoutSnapshot ? window.__naimageDebugLayoutSnapshot() : null");
  try {
  ...
  try {
    nativeCapture = await evaluate(
      client,
      `window.__naimageDebugCaptureGui ? window.__naimageDebugCaptureGui(${JSON.stringify(label)}, ${JSON.stringify(captureScope)}) : Promise.resolve({ ok: false, error: "capture hook missing" })`
    );
  } catch (error) {
...
async function seedClearCanvasProbe(client) {
  const state = await evaluate(client, `new Promise((resolve) => {
    ...
    setTimeout(() => resolve(window.__naimageDebugAgentState?.()), 320);
  })`);
  recordObservation("info", "agent-clear-canvas-seeded", { nodeCount: state?.nodeCount ?? 0 });
  await waitForExpression(client, "Number(window.__naimageDebugAgentState?.().nodeCount || 0) >= 2", 10000);
}
...
async function captureAgentToolSuiteProbe(client, targetId) {
  ...
  await evaluate(client, `window.__naimageDebugOpenSurface?.("main")`);
  await waitForExpression(client, "Boolean(window.__naimageAIDebug?.runSuite && window.__naimageDebugAgentState)", 10000);
  ...
  const suite = await evaluate(client, `window.__naimageAIDebug.runSuite()`);
...
async function captureRealAgentSuiteProbe(client, targetId) {
  ...
  await setWindowSize(client, targetId, 1280, 820);
  await evaluate(client, `window.__naimageDebugOpenSurface?.("main")`);
  await waitForExpression(client, "Boolean(window.__naimageAIDebug?.runRealAgentSuite && window.__naimageDebugAgentState)", 10000);
  ...
  const suite = await evaluate(client, `window.__naimageAIDebug.runRealAgentSuite({ liveImage: ${JSON.stringify(liveImage)}, skipImage: ${JSON.stringify(realAgentToolsOnly)} })`, suiteTimeoutMs);
...
async function captureAgentUiPromptSuiteProbe(client, targetId) {
  ...
  await setWindowSize(client, targetId, 1280, 820);
  await evaluate(client, `window.__naimageDebugOpenSurface?.("main")`);
  await waitForExpression(client, "Boolean(window.__naimageDebugAgentState && document.querySelector('.agent-composer textarea, .project-agent-composer textarea'))", 10000);
  ...
  const referenceImport = agentUiReferencePaths.length
    ? await evaluate(
        client,
        `window.__naimageDebugImportPathsAsReferences?.({ paths: ${JSON.stringify(agentUiReferencePaths)} })`,
        180000
      )
  ...
  if (agentUiReferencePaths.length && referenceImport?.ok !== false) {
    await waitForExpression(
      client,
      `Number(window.__naimageDebugAgentState?.().referenceImageCount || 0) >= ${agentUiReferencePaths.length}`,
      30000
    );
  }
...
async function captureAgentPosterBatchProbe(client, targetId, total = posterCount, agentCount = posterAgentCount, directCount = posterDirectCount) {
  ...
  await setWindowSize(client, targetId, 1280, 820);
  await evaluate(client, `window.__naimageDebugOpenSurface?.("main")`);
  await waitForExpression(client, "Boolean(window.__naimageAIDebug?.runPosterBatchSuite && window.__naimageDebugAgentState)", 10000);
  recordObservation("info", "agent-poster-batch-request", {
    ...payload,
    prompt: "AIDebug 执行真实 9:16 二次元写实风微海报批量生图，覆盖 Agent 与直接 image_gen 链路。"
  });
  await evaluate(client, `(() => {
    window.__naimagePosterBatchAsync = { done: false, ok: true, startedAt: new Date().toISOString() };
    Promise.resolve(window.__naimageAIDebug.runPosterBatchSuite(${JSON.stringify(payload)})).then(
      (result) => { window.__naimagePosterBatchAsync = { done: true, ok: true, startedAt: window.__naimagePosterBatchAsync?.startedAt, finishedAt: new Date().toISOString(), result }; },
      (error) => { window.__naimagePosterBatchAsync = { done: true, ok: false, startedAt: window.__naimagePosterBatchAsync?.startedAt, finishedAt: new Date().toISOString(), error: error instanceof Error ? error.message : String(error) }; }
    );
    return true;
  })()`);
  ...
  while (Date.now() - suiteStartedAt < suiteTimeoutMs) {
    asyncStatus = await evaluate(client, `(() => {
      const run = window.__naimagePosterBatchAsync || {};
      const heartbeat = window.__naimagePosterBatchHeartbeat || {};
  ...
  const suite = await evaluate(client, `window.__naimagePosterBatchAsync?.result`, Math.min(suiteTimeoutMs, 120000));
...
async function captureAgentExperienceImageProbe(client, targetId) {
  ...
  await evaluate(client, `window.__naimageDebugOpenSurface?.("main")`);
  await waitForExpression(client, "Boolean(window.__naimageAIDebug?.runExperienceImageSuite && window.__naimageDebugAgentState)", 10000);
  recordObservation("info", "agent-experience-image-request", {
    viewport: "540x700",
    prompt: "AIDebug 540px Agent 自主经验写入、读回并生图专项。"
  });
  const suite = await evaluate(client, `window.__naimageAIDebug.runExperienceImageSuite()`, suiteTimeoutMs);
...
async function captureAgentMixedStressProbe(client, targetId, rounds = stressRounds) {
  ...
  await setWindowSize(client, targetId, 1280, 820);
  await evaluate(client, `window.__naimageDebugOpenSurface?.("main")`);
  await waitForExpression(client, "Boolean(window.__naimageAIDebug?.runMixedStressSuite && window.__naimageDebugAgentState)", 10000);
  ...
  const suite = await evaluate(client, `window.__naimageAIDebug.runMixedStressSuite({ rounds: ${Number(rounds)} })`, suiteTimeoutMs);
  ...
  for (const index of focusTargetIndexes) {
    ...
    const focusCapture = await captureState(
      ...
      `window.__naimageDebugOpenSurface?.("main"); window.__naimageAIDebug?.selectNode?.({ id: ${JSON.stringify(id)} })`,
      { width: 884, height: 700 },
      {
        settingsOpen: false,
        historyOpen: false,
...
async function captureImageCollectionPersistenceProbe(client, targetId) {
  ...
  await setWindowSize(client, targetId, 1280, 820);
  await evaluate(client, `window.__naimageDebugOpenSurface?.("main")`);
  await waitForExpression(client, "Boolean(window.__naimageAIDebug?.runCanvasImageCollectionSuite && window.__naimageAIDebug?.fitCanvas && window.__naimageDebugAgentState)", 10000);
  ...
  if (stage === "write") {
    ...
    const persistenceGroupProof = await evaluate(
      ...
      `(async () => {
        ...
        const result = await window.__naimageAIDebug.nativeAssetDrop({ sourceNodeId, targetContainerId: targetNodeId, assetKey });
        ...
        const batchResizeAccepted = window.__naimageDebugResizeNode?.({ id: collectionNodeId, ...batchRequested }) === true;
        const layoutResizeAccepted = window.__naimageDebugResizeNode?.({ id: targetNodeId, ...layoutRequested }) === true;
        await new Promise((resolve) => window.setTimeout(resolve, 520));
        let after = window.__naimageDebugAgentState?.();
        ...
        for (const node of visibleNodes) {
          ...
          packed = window.__naimageDebugMoveNode?.({ id: node.id, x: packX, y: packY }) === true && packed;
          packX += width + 140;
          rowHeight = Math.max(rowHeight, height);
        }
        await new Promise((resolve) => window.setTimeout(resolve, 420));
        after = window.__naimageDebugAgentState?.();
    ...
    const state = await evaluate(client, `window.__naimageDebugAgentState?.()`);
  ...
  await waitForExpression(client, "Number(window.__naimageDebugAgentState?.()?.nodeCount || 0) === 6", 15000);
  const persisted = readPersistenceState();
  const stateBefore = await evaluate(client, `window.__naimageDebugAgentState?.()`);
  ...
  const dissolveAfterRestart = await evaluate(
    ...
    `(async () => {
      ...
      const group = before?.layoutGroups?.find((item) => item.memberNodeIds?.includes(extractedId));
       ...
       const result = await window.__naimageDebugMoveContainerAsset({ sourceContainerId: extractedId, assetKey, clientX, clientY });
      await new Promise((resolve) => window.setTimeout(resolve, 420));
      let after = window.__naimageDebugAgentState?.();
      ...
      const packed = restoredBeforePacking
        ? window.__naimageDebugMoveNode?.({ id: extractedId, ...packedPosition }) === true
        : false;
  ...
  const continuation = await evaluate(
    ...
    `(async () => {
      const extractedId = ${JSON.stringify(extractedId)};
      const selected = await window.__naimageAIDebug.selectNode({ id: extractedId });
      const run = await window.__naimageAIDebug.runTool(${JSON.stringify({
        name: "image_gen",
      ...
      await new Promise((resolve) => window.setTimeout(resolve, 520));
      return { selected, run, state: window.__naimageDebugAgentState?.() };
    })()`
  );
...
async function captureContextPersistenceProbe(client, targetId) {
  ...
  await setWindowSize(client, targetId, 1280, 820);
  await evaluate(client, `window.__naimageDebugOpenSurface?.("main")`);
  await waitForExpression(client, "Boolean(window.__naimageAIDebug?.runTool && window.__naimageAIDebug?.chat && window.__naimageDebugAgentState)", 10000);
  ...
  if (stage === "write") {
    const saveDetail = await step("context-persistence-write-fast-memory-api", () => evaluate(
      ...
      `(async () => {
        ...
        const saved = await window.naimageAgent?.saveFastMemory?.({ ...scope, text });
        const read = await window.naimageAgent?.getFastMemory?.(scope);
        const tools = await window.naimageAgent?.tools?.();
    ...
    await step("context-persistence-write-experience-readback", async () => {
      ...
      const read = await evaluate(client, `window.__naimageAIDebug.runTool(${JSON.stringify({
        name: "experience",
        input: toolInput
      })})`);
    ...
    await step("context-persistence-fast-memory-api-after-restart", async () => {
      ...
      return evaluate(
        ...
        `(async () => {
          ...
          const state = window.__naimageDebugAgentState?.();
          const activeScope = { projectId: String(state?.activeProjectId || ""), conversationId: String(state?.activeConversationId || "") };
          const read = await window.naimageAgent?.getFastMemory?.(expectedScope);
          return {
    ...
    await step("context-persistence-agent-read", async () => {
      const beforeState = await evaluate(client, `window.__naimageDebugAgentState?.()`);
      ...
      const detail = await evaluate(client, `window.__naimageAIDebug.chat(${JSON.stringify(prompt)}, { skipAsk: true })`);
...
async function captureClearCanvasAgentProbe(client, targetId) {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    await evaluate(client, `window.__naimageDebugSendAgentPrompt?.("我需要清理下画布", { skipAsk: true })`);
    try {
      await waitForExpression(
        client,
        `(() => {
...
function contextMenuSuiteSetupExpression(menuKind) {
  return `new Promise((resolve) => {
    ...
    (async () => {
      ...
      window.__naimageContextMenuAudit = null;
      ...
      if (!menu) {
        ...
        window.__naimageContextMenuAudit = result;
        resolve(result);
        return;
      }
      await delay(220);
...
async function captureContextMenuSuiteProbe(client, targetId) {
  ...
  for (const scenario of scenarios) {
    ...
    const menuAudit = await evaluate(client, "window.__naimageContextMenuAudit || null", 10000);
...
async function captureAuthGateSuiteProbe(client, targetId) {
  ...
  const logoutProof = await evaluate(client, `(async () => {
    ...
    const openSurface = async (surface, selector) => {
      const opened = window.__naimageDebugOpenSurface?.(surface) === true;
    ...
    window.__naimageAuthProbe = proof;
    return proof;
  })()`, 30000);
  ...
  const authReloadScript = await client.send("Page.addScriptToEvaluateOnNewDocument", {
    source: `window.__naimageAuthReloadSentinel = ${JSON.stringify(authReloadSentinel)};`
  }, 5000);
  ...
  const refreshProof = await evaluate(client, `(() => {
    ...
    window.__naimageAuthProbe = { ...${JSON.stringify(logoutProof)}, refreshIsolationOk, refreshResidual: residual, refreshReload: ${JSON.stringify(authReloadProof)} };
    return window.__naimageAuthProbe;
  })()`);
  ...
  const registerProof = await evaluate(client, `(async () => {
    ...
    window.__naimageAuthProbe = { ...(window.__naimageAuthProbe || {}), registerSwitchOk };
  ...
  const errorProof = await evaluate(client, `(async () => {
    ...
    window.__naimageAuthProbe = { ...(window.__naimageAuthProbe || {}), errorFlowOk, errorMessage: message };
  ...
  const reloginProof = await evaluate(client, `(async () => {
    ...
    window.__naimageAuthReloginProbe = { ok, residual, valuesSet, pendingIsolationOk, workbenchVisible: Boolean(document.querySelector('.ide-shell')) };
    return window.__naimageAuthReloginProbe;
  })()`, 20000);
  await waitForExpression(client, "Boolean(document.querySelector('.ide-shell') && window.__naimageAuthReloginProbe?.ok)", 12000);
...
async function captureImageImportSuiteProbe(client, targetId) {
  ...
  await waitForExpression(
    client,
    "Boolean(window.__naimageAIDebug?.imageImportStatus && window.__naimageAIDebug?.cancelImageImports && window.__naimageAIDebug?.chat && window.__naimageAIDebug?.selectNodes && window.__naimageAIDebug?.mergeSelectedImages && window.__naimageDebugImportPathsToCanvas && window.__naimageDebugImportPathsAsSources && window.__naimageDebugImportPathsAsReferences)",
    10000,
  );
  ...
  const suite = await evaluate(client, `(async () => {
    const config = window.naimageConfig;
    ...
    const canvas = document.querySelector(".workflow-canvas");
    const canvasRect = canvas?.getBoundingClientRect?.();
    const blankNodeCount = Number(window.__naimageDebugAgentState?.()?.nodes?.length || 0);
    ...
    const canvasImport = await window.__naimageDebugImportPathsToCanvas({
      paths: [paths[0], paths[1]],
      clientX: (canvasRect?.left || 0) + 280,
      clientY: (canvasRect?.top || 0) + 240
    });
    const canvasNode = await waitForCondition(() => window.__naimageDebugAgentState?.()?.nodes?.find((node) => node.id === canvasImport?.containerId && node.assetCount === 2));
    const folderImport = await window.__naimageDebugImportPathsToCanvas({
    ...
    const folderState = window.__naimageDebugAgentState?.();
    ...
    if (folderGroup?.id) await window.__naimageAIDebug.selectNodes({ ids: [folderGroup.id], primaryId: folderGroup.id });
    const folderScopePrompt = "请确认当前文件夹组包含的两张原图，无需调用工具。";
    const folderScopeChat = folderGroup?.id
      ? await window.__naimageAIDebug.chat(folderScopePrompt, { skipAsk: true, timeoutMs: 30000 })
    ...
    const targetFolderState = window.__naimageDebugAgentState?.();
    ...
    const targetImportedChildren = targetFolderChildIds
      ...
      .filter(Boolean);
    const canvasState = window.__naimageDebugAgentState?.();
    const sourcesBefore = Number(canvasState?.sourceImageCount || 0);
    const referencesBefore = Number(canvasState?.referenceImageCount || 0);
    const sourceImport = await window.__naimageDebugImportPathsAsSources({ paths: ${JSON.stringify(occurrenceFixtureDirs)} });
    await waitForCondition(() => Number(window.__naimageDebugAgentState?.()?.sourceImageCount || 0) === sourcesBefore + 2);
    const referenceImport = await window.__naimageDebugImportPathsAsReferences({ paths: [paths[8], paths[9]] });
    await waitForCondition(() => Number(window.__naimageDebugAgentState?.()?.referenceImageCount || 0) === referencesBefore + 2);
    const stagedAttachmentState = window.__naimageDebugAgentState?.();
    await window.__naimageAIDebug.selectNodes({ ids: [] });
    const attachmentPrompt = "请确认本轮附件角色，无需调用工具，也不要生成图片。";
    const attachmentChat = await window.__naimageAIDebug.chat(attachmentPrompt, { skipAsk: true, timeoutMs: 30000 });
    const attachmentState = window.__naimageDebugAgentState?.();
    ...
    const mixedRoleState = window.__naimageDebugAgentState?.();
    ...
    const beforeLayerImport = window.__naimageDebugAgentState?.();
    ...
    const layerTargetImport = await window.__naimageDebugImportPathsToCanvas({
      paths: [paths[4], paths[5]],
      targetContainerId: layers?.subjectId,
      clientX: (canvasRect?.left || 0) + 420,
      clientY: (canvasRect?.top || 0) + 330
    });
    const layerGuardNode = await waitForCondition(() => window.__naimageDebugAgentState?.()?.nodes?.find((node) => node.id === layerTargetImport?.containerId && node.assetCount === 2));
    const afterLayerImport = window.__naimageDebugAgentState?.();
    ...
    for (let pass = 0; pass < 3; pass += 1) {
      const liveIds = new Set((window.__naimageDebugAgentState?.()?.nodes || []).map((node) => node.id));
    ...
    const folderCleanupOk = Boolean(await waitForCondition(() => {
      const ids = new Set((window.__naimageDebugAgentState?.()?.nodes || []).map((node) => node.id));
...
async function captureCanvasClaritySuiteProbe(client, targetId) {
  ...
  const fixtureSvg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="4096" height="3072" viewBox="0 0 4096 3072">
      ...
      <text x="2048" y="1380" text-anchor="middle" fill="#071017" font-family="Segoe UI, sans-serif" font-size="300" font-weight="800">NAIMAGE</text>
  ...
  await waitForExpression(
    client,
    "Boolean(window.__naimageAIDebug?.runTool && window.__naimageDebugImportPathsToCanvas && window.__naimageDebugSetViewport)",
    10000
  );
  ...
  const setup = await evaluate(client, `(async () => {
    ...
    const first = await window.__naimageDebugImportPathsToCanvas({
      paths: [${JSON.stringify(fixturePath)}],
      clientX: (rect?.left || 0) + 300,
      clientY: (rect?.top || 0) + 280
    });
    const second = await window.__naimageDebugImportPathsToCanvas({
      paths: [${JSON.stringify(fixturePath)}],
    ...
    window.__naimageDebugMoveNode?.({ id: firstId, x: baseX, y: baseY });
    window.__naimageDebugMoveNode?.({ id: secondId, x: baseX + 460, y: baseY + 40 });
    const connected = window.__naimageDebugConnectNodes?.({ sourceId: firstId, targetId: secondId });
    await debug.selectNode({ id: firstId });
    window.__naimageDebugSetViewport({ scale: 1, nodeId: firstId });
    await delay(420);
  ...
  const interaction = await evaluate(client, `(async () => {
    ...
    window.__naimageDebugSetViewport({ scale: 1, nodeId: ${JSON.stringify(setup.firstId)} });
    await delay(180);
    return {
  ...
  for (const scale of [0.5, 1, 1.5, 2.4]) {
    await evaluate(client, `window.__naimageDebugSetViewport?.({ scale: ${scale}, nodeId: ${JSON.stringify(setup.firstId)} })`);
    await waitForExpression(client, `Math.abs(Number(window.__naimageDebugAgentState?.().viewport?.scale || 0) - ${scale}) < 0.001`, 5000);
...
async function main() {
  if (imageCollectionPersistenceOnly && !persistenceStage && process.env.NAIMAGE_AIDEBUG_CHILD !== "1") {
    await runImageCollectionPersistenceSupervisor();
    return;
  }
  if (contextPersistenceOnly && !persistenceStage && process.env.NAIMAGE_AIDEBUG_CHILD !== "1") {
    await runContextPersistenceSupervisor();
    return;
  }
  if (cycles > 1 && process.env.NAIMAGE_AIDEBUG_CHILD !== "1") {
  ...
  await evaluate(client, `(async () => {
    window.__naimageAgentToolsProbe = await window.naimageAgent?.tools?.();
    return window.__naimageAgentToolsProbe;
  })()`);
  ...
  try {
    ...
    if (imageImportSuiteOnly) {
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "image-import-suite" }
      });
      return;
    }
    ...
    if (askUserContinuationSuiteOnly) {
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "ask-user-continuation-suite" }
      });
      return;
    }
    if (contextMenuSuiteOnly) {
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "context-menu-suite" }
      });
      return;
    }
    if (selectionCommandSuiteOnly) {
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "selection-command-suite" }
      });
      return;
    }
    if (canvasClaritySuiteOnly) {
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "canvas-clarity-suite" }
      });
      return;
    }
    if (performanceSuiteOnly) {
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "performance-suite", baselineOnly: true },
        reportAfterObservations: { performance },
        consoleAfterSummary: { performance }
      });
      return;
    }
    if (authGateSuiteOnly) {
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "auth-gate-suite" }
      });
      return;
    }
    if (uiSurfaceSuiteOnly) {
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "ui-surface-suite" }
      });
      return;
    }
    if (agentUiPromptSuiteOnly) {
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "agent-ui-prompt-suite", agentMode: mockAgent ? "mock-agent" : "real-agent", liveImage, prompt: agentUiPrompt },
        consoleMetadata: { mode: "agent-ui-prompt-suite", agentMode: mockAgent ? "mock-agent" : "real-agent", liveImage }
      });
      return;
    }
    if (realAgentSuiteOnly) {
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "real-agent-suite", agentMode: mockAgent ? "mock-agent" : "real-agent", liveImage }
      });
      return;
    }
    if (imageCollectionPersistenceOnly) {
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "canvas-image-collection-persistence", persistenceStage: persistenceStage || "supervised", persistenceStateFile, aidebugConfigDir },
        consoleMetadata: { mode: "canvas-image-collection-persistence", persistenceStage: persistenceStage || "supervised" }
      });
      return;
    }
    if (contextPersistenceOnly) {
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "context-persistence", persistenceStage: persistenceStage || "supervised", persistenceSentinel, persistenceStateFile, aidebugConfigDir },
        consoleMetadata: { mode: "context-persistence", persistenceStage: persistenceStage || "supervised" }
      });
      return;
    }
    if (posterBatchOnly) {
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "poster-batch", liveImage, posterCount, posterAgentCount, posterDirectCount, posterResolution, posterQuality }
      });
      return;
    }
    if (mixedStressOnly) {
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "mixed-stress", stressRounds }
      });
      return;
    }
    if (regionRedrawOnly) {
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "region-redraw", liveImage }
      });
      return;
    }
    if (cutoutOnly) {
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "cutout", liveImage }
      });
      return;
    }
    if (layerStackOnly) {
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "layer-stack", liveImage }
      });
      return;
    }
    if (imageCollectionOnly) {
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "canvas-image-collection", liveImage }
      });
      return;
    }
    if (imageRecoveryOnly) {
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "image-recovery" }
      });
      return;
    }
    if (imageOnly) {
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "image-only", imageRuns }
      });
      return;
    }
    if (agentOnly) {
      ...
      const artifactContextExpression = openSurfaceExpression("agent-timeline", `
        ...
        await delay(160);
        const beforeAutoNodeIds = new Set((window.__naimageDebugAgentState?.().nodes || []).map((node) => String(node.id || "")));
        window.__naimageAutoImageProbe = await window.__naimageAIDebug?.runTool?.({
          name: "image_gen",
        ...
        await delay(220);
        const afterAutoNodes = window.__naimageDebugAgentState?.().nodes || [];
        ...
        await delay(520);
        window.__naimageArtifactSceneIds = { sourceId, derivedId, targetId, variantId };
        ...
        window.__naimageDebugMoveNode?.({ id: sourceId, x: 160, y: 100 });
        window.__naimageDebugMoveNode?.({ id: variantId, x: 1260, y: 100 });
        window.__naimageDebugMoveNode?.({ id: derivedId, x: 160, y: 700 });
        window.__naimageDebugMoveNode?.({ id: targetId, x: 1260, y: 700 });
        ...
        await delay(320);
       {
         ...
         if (await dragToTargetBody(74)) {
           const connected = window.__naimageDebugAgentState?.().nodes?.find((node) => node.id === targetId);
           ...
           const originalCY = finalC?.y;
            ...
            const overlapMoved = window.__naimageDebugMoveNode?.({ id: targetId, x: Number(finalB?.x || 0) + Number(finalB?.width || 364) - overlapInset, y: Number(finalB?.y || 0) });
           await delay(80);
        ...
        window.__naimageEdgeCreationSequenceProbe = {
       ...
       window.__naimageRelationTypeProbe = await window.__naimageAIDebug?.runTool?.({
          name: "workflow",
           input: { operation: "connect_nodes", sourceId, targetId: variantId, relationType: "variant", brief: "验证变体成果关系。" }
        });
        await delay(120);
        window.__naimageEdgeCreationSequenceProbe.afterSecondRelationAt = performance.now();
        window.__naimageEdgeCreationSequenceProbe.afterSecondRelationKeys = Array.from(document.querySelectorAll('.edge.provenance')).map((edge) =>
          String(edge.getAttribute('data-source-id') || '') + '->' + String(edge.getAttribute('data-target-id') || '')
        );
       {
         ...
         const persistedZ = () => Object.fromEntries((window.__naimageDebugAgentState?.().nodes || []).map((node) => [
           String(node.id || ''),
         ...
         const moved = movingNode && window.__naimageDebugMoveNode?.({
           id: derivedId,
         ...
         if (movingNode) window.__naimageDebugMoveNode?.({ id: derivedId, x: Number(movingNode.x || 0), y: Number(movingNode.y || 0) });
         await delay(140);
         ...
         const selected = await window.__naimageAIDebug?.selectNode?.({ id: derivedId });
         await delay(120);
         ...
         const persistedZOrderStable = !zOrderDebugDataAvailable || JSON.stringify(beforePersistedZ) === JSON.stringify(selectedPersistedZ);
          window.__naimageEdgeStabilityProbe = {
         ...
         await delay(220);
         {
           ...
           const obstacleState = window.__naimageDebugAgentState?.().nodes?.find((node) => node.id === derivedId);
           ...
           const viewportScale = Math.max(0.01, Number(window.__naimageDebugAgentState?.().viewport?.scale || 1));
           const obstacleMove = screenMidpoint && obstacleBeforeBox && obstacleState
             ? window.__naimageDebugMoveNode?.({
                 id: derivedId,
           ...
           if (obstacleState) {
             window.__naimageDebugMoveNode?.({ id: derivedId, x: Number(obstacleState.x || 0), y: Number(obstacleState.y || 0) });
           }
           await delay(160);
           ...
           window.__naimageEdgeObstacleProbe = {
         ...
         {
           const stateBeforeCross = window.__naimageDebugAgentState?.() || {};
           ...
           const movedC = nodeC ? window.__naimageDebugMoveNode?.({ id: targetId, x: Number(nodeC.x || 0), y: topY }) : false;
           const movedD = nodeD ? window.__naimageDebugMoveNode?.({ id: variantId, x: Number(nodeD.x || 0), y: bottomY }) : false;
           await delay(180);
           await window.__naimageAIDebug?.fitCanvas?.();
           await delay(260);
           ...
           const beforeSecondKeys = Array.isArray(window.__naimageEdgeCreationSequenceProbe?.beforeSecondRelationKeys)
             ? window.__naimageEdgeCreationSequenceProbe.beforeSecondRelationKeys
             : [];
           const afterSecondKeys = Array.isArray(window.__naimageEdgeCreationSequenceProbe?.afterSecondRelationKeys)
             ? window.__naimageEdgeCreationSequenceProbe.afterSecondRelationKeys
             : [];
           ...
           const reloadApiKeys = Object.keys(window.__naimageAIDebug || {}).filter((key) => /reload|persist|save.*project|load.*project/i.test(key));
           ...
           window.__naimageEdgeOrderProbe = {
             ...
             persistenceReloadGap: persistenceReloadAvailable
               ...
               : 'No scoped save/reload project API is exposed by window.__naimageAIDebug; a full location reload would destroy the inline probe and destabilize the remaining suite.',
             originalPositions: {
        ...
        await delay(260);
         {
          ...
          if (canvas && appliedPanY > 0) {
            canvas.dispatchEvent(new CustomEvent('naimage-debug-pan', { bubbles: true, detail: { dx: 0, dy: appliedPanY } }));
            await delay(220);
          }
          ...
          const visibleRatio = (box) => {
           ...
           const selectedIds = [...(window.__naimageDebugAgentState?.().selectedNodeIds || [])];
           const viewport = window.__naimageDebugAgentState?.().viewport || null;
           const thresholds = {
             nodeVisibleRatioMin: 0.98,
             nodeScreenWidthMin: 96,
             nodeScreenHeightMin: 64,
           ...
           window.__naimageProvenanceSceneProbe = {
      ...
      if (!artifactContextCapture.state?.zOrderDebugDataAvailable) {
        recordObservation("info", "z-order-debug-data-pending", {
         ...
         requiredMainDebugFields: ["window.__naimageDebugAgentState().nodes[].zOrder"],
       ...
       results.push(artifactContextCapture);
      {
        ...
        if (originalPositions?.C && originalPositions?.D) {
          await evaluate(client, `(async () => {
            window.__naimageDebugMoveNode?.({ id: ${JSON.stringify(String(originalPositions.C.id || ""))}, x: ${JSON.stringify(Number(originalPositions.C.x || 0))}, y: ${JSON.stringify(Number(originalPositions.C.y || 0))} });
            window.__naimageDebugMoveNode?.({ id: ${JSON.stringify(String(originalPositions.D.id || ""))}, x: ${JSON.stringify(Number(originalPositions.D.x || 0))}, y: ${JSON.stringify(Number(originalPositions.D.y || 0))} });
            await window.__naimageAIDebug?.fitCanvas?.();
            return true;
          })()`, 10000);
        }
       ...
       const multiSelectionExpression = openSurfaceExpression("agent-timeline", `
        ...
        await delay(240);
        {
          ...
          window.__naimageMultiSelectionSceneProbe = {
      ...
      const middleMarqueeSelectionExpression = openSurfaceExpression("agent-timeline", `
        {
          ...
          if (firstId) await window.__naimageAIDebug?.selectNode?.({ id: firstId });
          await delay(120);
          const before = [...(window.__naimageDebugAgentState?.().selectedNodeIds || [])];
          ...
          window.__naimageMiddleMarqueeSelectionProbe = {
            ok: Boolean(
      ...
      const ctrlNodeSelectionExpression = openSurfaceExpression("agent-timeline", `
        {
          ...
          const afterAdd = [...(window.__naimageDebugAgentState?.().selectedNodeIds || [])];
          const removed = await ctrlClick(removeNode, 77);
          const afterRemove = [...(window.__naimageDebugAgentState?.().selectedNodeIds || [])];
          ...
          window.__naimageCtrlNodeSelectionProbe = {
            ok: Boolean(
      ...
      const canvasPanSelectionExpression = openSurfaceExpression("agent-timeline", `
        {
          ...
          const selectionBefore = window.__naimageDebugAgentState?.().selectedNodeIds || [];
          ...
          const selectionAfter = window.__naimageDebugAgentState?.().selectedNodeIds || [];
          window.__naimageCanvasLeftPanProbe = {
            before,
            after,
            moved: before !== after,
            selectionBefore,
      ...
      const stickySelectionExpression = openSurfaceExpression("agent-timeline", `
        {
          ...
          const snapshots = [];
          const record = (label) => snapshots.push({ label, ids: [...(window.__naimageDebugAgentState?.().selectedNodeIds || [])] });
          selectedNode?.click();
          ...
          window.__naimageStickySelectionProbe = {
      ...
      const pruneSelectionExpression = openSurfaceExpression("agent-timeline", `
        {
          ...
          if (removedId) window.__naimageDebugApplyAgentActions?.([{ type: 'workflow.node.delete', id: removedId, mode: 'only' }]);
          await delay(160);
          const after = [...(window.__naimageDebugAgentState?.().selectedNodeIds || [])];
          const expected = before.filter((id) => id !== removedId);
          window.__naimageSelectionPruneProbe = {
      ...
      const singleSelectionSwitchExpression = openSurfaceExpression("agent-timeline", `
        {
          ...
          unselectedNode?.click();
          await delay(100);
          const switchedState = window.__naimageDebugAgentState?.() || {};
          ...
          clearButton?.click();
          await delay(100);
          const afterExplicitClear = [...(window.__naimageDebugAgentState?.().selectedNodeIds || [])];
          ...
          window.__naimageSingleSelectionSwitchProbe = {
            ok: Boolean(
              before.length === 1 &&
     ...
     const imageContainerExpression = openSurfaceExpression("agent-timeline", `
        ...
        await delay(140);
       {
         ...
         await delay(140);
         window.__naimageCanvasMenuProbe = { texts: Array.from(document.querySelectorAll(".canvas-context-menu button")).map((item) => String(item.textContent || "").replace(/\\s+/g, " ").trim()) };
        ...
        await delay(220);
       {
         const containerId = window.__naimageDebugAgentState?.().nodes?.find((node) => node.imageContainer)?.id || "";
          window.__naimageLegacyContainerId = containerId;
          window.__naimageContainerImportProbe = await window.__naimageDebugImportPathsToCanvas?.({ paths: [${JSON.stringify(dragFixtureDir)}], targetContainerId: containerId });
          const importDeadline = Date.now() + 8000;
          let importedContainer = null;
          while (Date.now() < importDeadline) {
            importedContainer = window.__naimageDebugAgentState?.().nodes?.find((node) => node.id === containerId && node.imageContainer) || null;
            if (window.__naimageContainerImportProbe?.ok && importedContainer?.assetCount === 2) break;
            await delay(40);
          }
          window.__naimageContainerImportSettleProbe = {
            ok: Boolean(window.__naimageContainerImportProbe?.ok && importedContainer?.assetCount === 2),
            containerId,
        ...
        {
          const before = window.__naimageDebugAgentState?.();
          const container = before?.nodes?.find((node) => node.id === window.__naimageLegacyContainerId && node.imageContainer);
          const assetKey = container?.assets?.[0]?.path || container?.assets?.[0]?.assetUrl || "";
          window.__naimageContainerMoveProbe = await window.__naimageDebugMoveContainerAsset?.({ sourceContainerId: container?.id || "", assetKey });
          await delay(360);
          ...
          window.__naimageContainerSplitProbe = {
            ok: Boolean(window.__naimageContainerMoveProbe?.ok && imported.length === 2 && imported.some((node) => node.id === window.__naimageLegacyContainerId)),
        ...
        await delay(220);
        {
          ...
          const target = imported.find((node) => node.id === window.__naimageLegacyContainerId) || imported[0];
          ...
          const result = await window.__naimageAIDebug?.nativeAssetDrop?.({ sourceNodeId: source?.id || "", targetContainerId: target?.id || "", assetKey });
          await delay(420);
          const after = window.__naimageDebugAgentState?.();
          ...
          window.__naimageContainerGroupProbe = {
        ...
        await delay(220);
        {
          const group = window.__naimageContainerGroupProbe?.group;
          const containerId = group?.hostNodeId || "";
           const roleResult = await window.__naimageAIDebug?.setContainerRole?.({ id: containerId, role: "reference" });
           await delay(220);
           window.__naimageContainerAgentRunProbe = await window.__naimageAIDebug?.runTool?.({
             name: "image_gen",
            ...
            await delay(360);
           const generated = window.__naimageDebugAgentState?.().nodes?.find((node) => node.parentId === containerId && /referenceImages:\\s*2/i.test(String(node.prompt || "")));
           const referenceMatch = String(generated?.prompt || "").match(/referenceImages:\\s*(\\d+)/i);
           window.__naimageContainerAgentProbe = {
             ok: Boolean(window.__naimageContainerAgentRunProbe?.ok && generated && Number(referenceMatch?.[1] || 0) === 2),
             roleSet: roleResult?.ok === true,
             selectedNodeId: containerId,
             actionCount: Number(window.__naimageContainerAgentRunProbe?.actionCount || 0),
             ...
             envelopeSummary: String(window.__naimageContainerAgentRunProbe?.envelope?.summary || "")
           };
          if (generated?.id) window.__naimageDebugApplyAgentActions?.([{ type: "workflow.node.delete", id: generated.id, mode: "only" }]);
        }
        await delay(220);
        {
          const before = window.__naimageDebugAgentState?.();
          const group = before?.layoutGroups?.find((item) => item.id === window.__naimageContainerGroupProbe?.group?.id);
          const memberId = group?.memberNodeIds?.find((id) => id !== group.hostNodeId) || "";
           ...
           const extractionClientY = hostCenterY <= canvasCenterY
             ...
             : (canvasBox?.top || 0) + 72;
           const result = await window.__naimageDebugMoveContainerAsset?.({
           ...
           const separatedDistance = extractedMember && extractedHost
             ...
             : 0;
           window.__naimageContainerDissolveProbe = {
           ...
           await window.__naimageAIDebug?.fitCanvas?.();
           await delay(420);
           const regroup = await window.__naimageAIDebug?.nativeAssetDrop?.({ sourceNodeId: memberId, targetContainerId: group?.hostNodeId || "", assetKey });
          await delay(420);
          const finalState = window.__naimageDebugAgentState?.();
          const finalGroup = finalState?.layoutGroups?.find((item) => item.hostNodeId === group?.hostNodeId && item.memberNodeIds?.includes(memberId));
          window.__naimageContainerFinalGroupProbe = {
        ...
        await delay(220);
       window.__naimageReferenceDropProbe = await window.__naimageDebugImportPathsAsReferences?.({ paths: [${JSON.stringify(dragFixtureDir)}] });
       await delay(260);
        document.querySelector(".project-agent-references")?.click();
        await delay(180);
        window.__naimageReferenceCapacityProbe = { slots: document.querySelectorAll(".reference-picker-dialog .reference-slot").length };
        ...
        window.__naimageReferencePickerCloseProbe = {
     ...
     const layeredPngExpression = openSurfaceExpression("agent-timeline", `
        window.__naimageDebugApplyAgentActions?.([{ type: "workflow.canvas.clear", mode: "all" }]);
        await delay(120);
        window.__naimageLayersProbe = await window.__naimageAIDebug?.runTool?.({
          name: "image_gen",
          input: {
            operation: "layers",
        ...
        let layerState = window.__naimageAIDebug?.layerState?.() || { selectedNodeId: "", nodes: [] };
        while (Date.now() < layerDeadline && !(layerState.nodes.length === 3 && layerState.nodes.every((node) => node.assetCount === 1))) {
          await delay(180);
          layerState = window.__naimageAIDebug?.layerState?.() || { selectedNodeId: "", nodes: [] };
        }
        window.__naimageLayerStateProbe = layerState;
        const menuNodeId = layerState.selectedNodeId || layerState.nodes.at(-1)?.id || "";
        await delay(240);
        window.__naimageLayerMenuProbe = await window.__naimageAIDebug?.openNodeMenu?.({ id: menuNodeId });
        await delay(160);
      `);
      ...
      const editVariantsExpression = openSurfaceExpression("agent-timeline", `
        window.__naimageDebugApplyAgentActions?.([{ type: "workflow.canvas.clear", mode: "all" }]);
        await delay(120);
        window.__naimageEditSourceProbe = await window.__naimageAIDebug?.runTool?.({
          name: "image_gen",
          input: { operation: "generate", prompt: "AIDebug 蓝色运动鞋产品图。", ratio: "1:1", resolution: "720P", count: 1, quality: "auto", brief: "生成款式来源图。" }
        });
        await delay(260);
        window.__naimageEditSourceId = window.__naimageDebugAgentState?.().selectedNodeId || "";
        window.__naimageVariantsProbe = await window.__naimageAIDebug?.runTool?.({
          name: "image_gen",
          selectedNodeId: window.__naimageEditSourceId,
          input: { operation: "variants", parentId: window.__naimageEditSourceId, prompt: "设计四种不同鞋带和配色款式，保持鞋型一致。", ratio: "1:1", resolution: "720P", count: 4, quality: "auto", brief: "生成四种独立款式。" }
        });
        await delay(320);
        window.__naimageReplaceProbe = await window.__naimageAIDebug?.runTool?.({
          name: "image_gen",
          selectedNodeId: window.__naimageEditSourceId,
          input: { operation: "replace", parentId: window.__naimageEditSourceId, prompt: "把鞋带替换成红色丝带，其他区域保持不变。", ratio: "1:1", resolution: "720P", count: 1, quality: "auto", brief: "替换鞋带元素。" }
        });
        await delay(420);
        const referenceSource = window.__naimageDebugAgentState?.().nodes?.find((node) => node.id === window.__naimageEditSourceId)?.assets?.[0];
        window.__naimageReferenceProbe = await window.__naimageAIDebug?.runTool?.({
          name: "image_gen",
          selectedNodeId: "",
      ...
      const isolationExpression = openSurfaceExpression("agent-timeline", `
        {
          const stateBefore = window.__naimageDebugAgentState?.();
          const clearSentinel = "AIDebug-clear-scope-" + Date.now();
          const clearWrite = await window.__naimageAIDebug?.chat?.(
          ...
          await delay(420);
          const stateAfterClear = window.__naimageDebugAgentState?.();
          const clearRead = await window.__naimageAIDebug?.runTool?.({
            name: "experience",
            input: { action: "read", brief: "验证当前会话经验已清除。" }
          });
          const clearOutput = String(clearRead?.envelope?.visibleOutput || clearRead?.envelope?.summary || "");
          window.__naimageIsolationProbe = {
            clear: {
              ok: Boolean(
                clearWrite?.ok &&
                experienceUiOk &&
          ...
          const conversationWrite = await window.__naimageAIDebug?.runTool?.({
            name: "experience",
            input: { action: "add", title: "会话隔离专项", text: conversationSentinel, rating: "note", brief: "记录旧会话专项经验。" }
          });
          const beforeConversation = window.__naimageDebugAgentState?.();
          document.querySelector('button[title="新建会话"]')?.click();
          await delay(120);
          document.querySelector(".confirm-dialog footer button:last-child")?.click();
          await delay(320);
          const afterConversation = window.__naimageDebugAgentState?.();
          const conversationRead = await window.__naimageAIDebug?.runTool?.({
            name: "experience",
          ...
          window.__naimageIsolationProbe.conversation = {
            ok: Boolean(
          ...
          const projectWrite = await window.__naimageAIDebug?.runTool?.({
            name: "experience",
            input: { action: "add", title: "项目隔离专项", text: projectSentinel, rating: "note", brief: "记录旧项目专项经验。" }
          });
          const beforeProject = window.__naimageDebugAgentState?.();
          ...
          await delay(720);
          const afterProject = window.__naimageDebugAgentState?.();
          const projectRead = await window.__naimageAIDebug?.runTool?.({
            name: "experience",
          ...
          window.__naimageIsolationProbe.project = {
            ok: Boolean(
              projectWrite?.ok &&
    ...
    if (!legacyFullSuite) {
      ...
      const modelPickerExpression = openSurfaceExpression("model-config", `
        {
          ...
          const standard = [];
          for (let index = 0; index < 5; index += 1) standard.push(await window.naimageServer?.models?.({ forceRefresh: false }));
          const forced = await window.naimageServer?.models?.({ forceRefresh: true });
          const afterForce = await window.naimageServer?.models?.({ forceRefresh: false });
          const agentFirst = await window.naimageAgent?.listModels?.({ provider: "agent", settings: {} });
          const agentSecond = await window.naimageAgent?.listModels?.({ provider: "agent", settings: {} });
          ...
          window.__naimageModelCacheProbe = {
          ...
          if (list) {
            ...
            await delay(100);
            window.__naimageModelPickerScrollProbe = { before, after: list.scrollTop, moved: list.scrollTop > before };
          } else {
            window.__naimageModelPickerScrollProbe = { before: 0, after: 0, moved: false };
          }
        }
      `);
      ...
      finishSuiteRun({
        results,
        reportMetadata: { mode: "canvas-only-smoke" }
      });
      return;
    }
    ...
    results.push(await captureState(client, target.id, "workspace-resize-1280", "window.__naimageDebugOpenSurface?.('main'); window.__naimageDebugResizeWorkspace?.({ inspectorWidth: 300, agentWidth: 360 }); undefined", { width: 1280, height: 820 }, { settingsOpen: false, historyOpen: false, modalOpen: false, accountOpen: false, agentWidth: 40, titlebarOverlay: true, titlebarBrandVisible: true, rightCollapseAligned: true, compactMainGridOk: true }));
    ...
    finishSuiteRun({
      results,
      reportMetadata: { imageRuns, stressRounds },
