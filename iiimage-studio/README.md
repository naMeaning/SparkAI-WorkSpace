...
IIIMAGE STUDIO 是独立维护的 Electron + React 傻瓜式 AI 生图工作台。右侧项目 Agent 是唯一任务入口，可折叠为窄栏但不会被卸载或创建子 Agent；无限画布只展示图片成果、图片组、分层栈及其来源关系。`image_gen` 是唯一图片执行工具，默认使用 `gpt-image-2`，支持文字/参考图生图、连续或最多 10 张并行图片、元素替换、单图多款、分层 PNG、AI 抠图和带蒙版区域重绘。Agent 可在内部使用文件查看、命令、联网检索、经验与 compact 等通用能力，但不会把这些过程放到画布。设置中可以直接编辑/恢复主 Agent 纯文本 Prompt；Agent 标题栏可以编辑当前项目当前会话的 FastMemory，Prompt、工具 Schema、会话压缩摘要和 FastMemory 彼此独立。
外部图片和文件夹可直接拖入：拖到聊天区域会加入参考图容器，拖到画布会复制进当前项目图片库并归入可视图片容器；用户原始文件不会被修改。

## 1.0.1 发布版

- Windows 10/11 x64 安装包：`release/iiimage-Studio-Setup-1.0.1-x64.exe`
- SHA-256：`5213423f5af737d6bbce3e3f389c9e194caac7501b796f07118e6c521a793145`
- 33 张真实精选案例通过 Forge Release 的 `iiimage-Studio-Showcase-1.0.0.zip` 分发，不写入普通 Git 历史。
- 安装器支持选择路径，并创建带正式图标的桌面与开始菜单快捷方式。
- 客户端支持签名校验、断点下载和重启更新；需要重新安装的大版本由软件更新中心下载完整安装包。
- 当前构建未配置商业代码签名证书，Authenticode 状态为 `NotSigned`；只能从官方 Forge 获取并在安装前核对 SHA-256。
- 完整安装、卸载和数据目录说明见 [docs/INSTALLATION.md](./docs/INSTALLATION.md)，本次发布内容见 [docs/RELEASE_1.0.1.md](./docs/RELEASE_1.0.1.md)。
