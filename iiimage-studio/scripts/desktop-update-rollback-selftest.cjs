...
const temporaryRoot = persistedMode ? suppliedConfigDir : mkdtempSync(path.join(os.tmpdir(), "iiimage-update-rollback-test-"));

process.env.IIIMAGE_AGENT_PROTOCOL_SELFTEST = "1";
process.env.IIIMAGE_CONFIG_DIR = temporaryRoot;
if (!persistedMode) process.env.IIIMAGE_UPDATE_ROLLBACK_SELFTEST = "1";
else delete process.env.IIIMAGE_UPDATE_ROLLBACK_SELFTEST;
...
try {
  if (!persistedMode) {
    ...
    const updatePath = path.join(updateDir, `iiimage-Studio-Restart-Update-${version}-x64.asar`);
    ...
    writeFileSync(path.join(temporaryRoot, "updates", "update-state.json"), `${JSON.stringify({
      progress: {
        stage: "applying",
        message: "iiimage Studio 即将重启并完成更新…",
        version,
        kind: "restart",
  ...
  if (!persistedMode) {
    ...
    delete childEnvironment.IIIMAGE_UPDATE_ROLLBACK_SELFTEST;
