...
export async function captureAskUserContinuationSuite(context) {
  ...
  await waitForExpression(
    client,
    "Boolean(window.__iiimageAIDebug?.chat && window.__iiimageAIDebug?.newConversation && window.__iiimageAIDebug?.addReferencePickerPaths && window.__iiimageDebugAgentState)",
    10000
  );
  const results = [];
  const checks = {};
  ...
  const cleanStart = await evaluate(client, `(async () => {
    const conversation = await window.__iiimageAIDebug.newConversation();
    await window.__iiimageAIDebug.runTool({ name: "workflow", input: { operation: "clear_canvas", brief: "清理 AskUser continuation 专项画布。" } });
    await window.__iiimageAIDebug.seedCanvas({ count: 1, fileBacked: true });
    return { conversation, state: window.__iiimageDebugAgentState?.() };
  })()`);
  ...
  const requirementCreated = requirementSourceId ? await evaluate(client, `(async () => {
    await window.__iiimageAIDebug.openRequirementForSource({ id: ${JSON.stringify(requirementSourceId)} });
    ...
    if (!textarea) return { ok: false, error: "requirement editor unavailable", state: window.__iiimageDebugAgentState?.() };
    ...
    while (performance.now() < createdDeadline) {
      const requirement = window.__iiimageDebugAgentState?.().nodes?.find((node) => node.type === "requirement" && node.parentId === ${JSON.stringify(requirementSourceId)});
      if (requirement) return { ok: true, requirement, state: window.__iiimageDebugAgentState?.() };
      await new Promise((resolve) => setTimeout(resolve, 50));
    }
    return { ok: false, error: "requirement creation timed out", state: window.__iiimageDebugAgentState?.() };
  ...
  const runPickerContinuation = async (kind, fixturePath) => {
    ...
    const started = await evaluate(client, `window.__iiimageAIDebug.chat(${JSON.stringify(prompt)}, { timeoutMs: 30000 })`, 40000);
    ...
    const added = await evaluate(client, `window.__iiimageAIDebug.addReferencePickerPaths({ paths: [${JSON.stringify(fixturePath)}] })`, 20000);
  ...
  results.push(await captureState(client, targetId, "ask-user-source-resumed", "window.__iiimageAIDebug?.fitCanvas?.()", { width: 1280, height: 820 }, {
    modalOpen: false,
    askUserOpen: false,
  ...
  const structuredConfirm = await evaluate(client, `(async () => {
    const ask = await window.__iiimageAIDebug.chat("AIDEBUG_ASK_CONFIRM ${marker} 验证结构化批量策略选项。", { timeoutMs: 30000 });
  ...
  const cancellation = await evaluate(client, `(async () => {
    const ask = await window.__iiimageAIDebug.chat("AIDEBUG_ASK_CONFIRM ${marker} 请先确认这次测试任务。", { timeoutMs: 30000 });
    ...
    while (performance.now() < closeDeadline && window.__iiimageDebugAgentState?.().pendingAgentExecution) await new Promise((resolve) => setTimeout(resolve, 50));
    const before = window.__iiimageDebugAgentState?.() || {};
    const beforeIds = new Set((before.nodes || []).filter((node) => node.type === "image").map((node) => node.id));
    const fresh = await window.__iiimageAIDebug.chat("取消旧任务后，这是一条新的独立请求：请生成一张 1:1 极简蓝白测试图。", { timeoutMs: 40000 });
    const state = window.__iiimageDebugAgentState?.() || {};
  ...
  const conversationIsolation = await evaluate(client, `(async () => {
    const ask = await window.__iiimageAIDebug.chat("AIDEBUG_ASK_CLARIFY ${marker} 新会话隔离前的旧任务。", { timeoutMs: 30000 });
    const before = window.__iiimageDebugAgentState?.() || {};
    const created = await window.__iiimageAIDebug.newConversation();
  ...
  const persistenceStarted = await evaluate(client, `window.__iiimageAIDebug.chat("AIDEBUG_ASK_SOURCE ${marker} 请保存并在界面重载后恢复这个 SOURCE 请求。", { timeoutMs: 30000 })`, 40000);
  await waitForExpression(client, "Boolean(window.__iiimageDebugAgentState?.().pendingAgentExecution?.kind === 'source_images' && window.__iiimageDebugAgentState?.().referencePickerOpen)", 6000);
  await new Promise((resolve) => setTimeout(resolve, 1500));
  phase("reload-renderer-with-pending-request");
  await client.send("Page.reload", { ignoreCache: true });
  await waitForExpression(client, "Boolean(window.__iiimageAIDebug?.addReferencePickerPaths && window.__iiimageDebugAgentState)", 15000);
  await waitForExpression(
    client,
    "Boolean(window.__iiimageDebugAgentState?.().pendingAgentExecution?.kind === 'source_images' && document.querySelector('[data-ui-surface=\"reference-picker\"]') && Array.from(document.querySelectorAll('[data-ui-surface=\"reference-picker\"] [aria-label]')).some((item) => item.getAttribute('aria-label') === '添加原图'))",
    10000
  );
  ...
  await waitForExpression(client, "!window.__iiimageDebugAgentState?.().pendingAgentExecution && !window.__iiimageDebugAgentState?.().referencePickerOpen", 5000);

  phase("staged-multi-source-result-layout");
  const stagedTaskResult = await evaluate(client, `(async () => {
    await window.__iiimageAIDebug.newConversation();
    const seeded = await window.__iiimageAIDebug.seedCanvas({ count: 2, fileBacked: true });
    const sourceIds = (seeded?.state?.nodes || []).filter((node) => node.type === "image").map((node) => node.id).slice(-2);
    await window.__iiimageAIDebug.selectNodes({ ids: sourceIds, primaryId: sourceIds[0] });
    const before = window.__iiimageDebugAgentState?.() || {};
    const beforeIds = new Set((before.nodes || []).map((node) => node.id));
    const ask = await window.__iiimageAIDebug.chat(
      "AIDEBUG_ASK_CONFIRM ${marker} 请基于当前两个 SOURCE 分别生成一张 1:1 蓝金商品测试图。",
      { timeoutMs: 30000 }
    );
    const openDeadline = performance.now() + 5000;
    while (performance.now() < openDeadline && !document.querySelector(".ask-user-dialog")) await new Promise((resolve) => setTimeout(resolve, 50));
    const pending = window.__iiimageDebugAgentState?.().pendingAgentExecution;
    ...
    return { ok: false, ask, pending, state: window.__iiimageDebugAgentState?.() };
  })()`, 110000);
  checks.stagedMultiSourceResultLayout = Boolean(stagedTaskResult?.ok);

  phase("capture-final");
  const finalCapture = await captureState(client, targetId, "ask-user-continuation-final", "window.__iiimageAIDebug?.fitCanvas?.()", { width: 1280, height: 820 }, {
    modalOpen: false,
    askUserOpen: false,
    referencePickerOpen: false,
    agentIdle: true,
