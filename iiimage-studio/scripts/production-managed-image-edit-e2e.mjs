...
const evidenceDir = path.resolve(process.env.IIIMAGE_EDIT_E2E_OUTPUT || defaultEvidenceDir);
const settingsPath = path.resolve(
  process.env.IIIMAGE_SETTINGS_PATH || path.join(repoRoot, "config", "app-settings.json"),
);
const sourcePath = path.resolve(
  process.env.IIIMAGE_EDIT_SOURCE
    || path.join(
      repoRoot,
      ".diagnostics",
...
const timeoutMs = Number(process.env.IIIMAGE_EDIT_TIMEOUT_MS || 300_000);
const basePrompt = process.env.IIIMAGE_EDIT_PROMPT || [
