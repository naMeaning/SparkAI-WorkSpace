...
export function parseAidebugOptions(argv, env, context) {
  ...
  const explicitDevPort = firstValue(devPortArg) || env.IIIMAGE_AIDEBUG_DEV_PORT || "";
  ...
  const explicitDebugPort = firstValue(portArg) || env.IIIMAGE_REMOTE_DEBUGGING_PORT || "";
  const debugPort = Number(explicitDebugPort || defaultDebugPort);
  const keepOpen = has("--keep-open");
  const liveImage = has("--live-image") || env.IIIMAGE_AIDEBUG_LIVE_IMAGE === "1";
  ...
  const realAgent = realAgentSuiteOnly || realAgentToolsOnly || agentUiPromptSuiteOnly || has("--real-agent") || env.IIIMAGE_AIDEBUG_REAL_AGENT === "1";
  const liveConfig = liveImage || realAgent || has("--live-config") || env.IIIMAGE_AIDEBUG_LIVE_CONFIG === "1";
  const mockAgent =
    has("--mock-agent") ||
    env.IIIMAGE_AIDEBUG_MOCK_AGENT === "1" ||
    (!realAgent && !liveImage);
  ...
  const aidebugConfigDir = resolveFromCwd(fullValue(aidebugConfigDirArg) || env.IIIMAGE_AIDEBUG_CONFIG_DIR || join(runDir, "config"));
  ...
  const agentUiPrompt = String(
    env.IIIMAGE_AIDEBUG_AGENT_UI_PROMPT ||
  ...
  const agentUiFollowupPrompt = String(
    env.IIIMAGE_AIDEBUG_AGENT_UI_FOLLOWUP ||
  ...
  const agentUiReferencePaths = (() => {
    const raw = String(
      env.IIIMAGE_AIDEBUG_AGENT_UI_REFERENCES ||
  ...
  const agentUiSingleOnly = has("--agent-ui-single") || env.IIIMAGE_AIDEBUG_AGENT_UI_SINGLE === "1";
