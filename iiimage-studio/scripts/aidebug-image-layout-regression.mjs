export async function runCanvasLayoutMutationRegression({ client, evaluate, fixturePath, fixturePaths = [] }) {
  return evaluate(
    ...
    `(async () => {
      ...
      const clear = async () => {
        const result = await window.__iiimageAIDebug.runTool({
          name: "workflow",
      ...
      const groupInto = async (sourceNodeId, targetNodeId) => {
        ...
        const result = await window.__iiimageAIDebug.nativeAssetDrop({ sourceNodeId, targetContainerId: targetNodeId, assetKey: assetKey(source) });
        await delay(300);
      ...
      const deleteNode = async (id) => {
        const applied = window.__iiimageDebugApplyAgentActions?.([{ type: "workflow.node.delete", id, mode: "only" }]);
        await delay(520);
        return applied === true;
      };
      ...
      await clear();
      {
        ...
        const canvasRect = document.querySelector('.workflow-canvas')?.getBoundingClientRect();
        const move = await window.__iiimageDebugMoveContainerAsset({ sourceContainerId: hostId, assetKey: assetKey(host), clientX: (canvasRect?.left || 0) + 720, clientY: (canvasRect?.top || 0) + 480 });
        await delay(420);
        const after = state();
      ...
      {
        const canvasRect = document.querySelector('.workflow-canvas')?.getBoundingClientRect();
        const imported = await window.__iiimageDebugImportPathsToCanvas?.({ paths: [${JSON.stringify(fixturePath)}], clientX: (canvasRect?.left || 0) + 260, clientY: (canvasRect?.top || 0) + 220 });
        await delay(420);
        const before = state();
        ...
        const beforeCausality = causality(before?.nodes);
        const move = await window.__iiimageDebugMoveContainerAsset?.({ sourceContainerId: classic?.id || "", assetKey: assetKey(classic), clientX: (canvasRect?.left || 0) + 690, clientY: (canvasRect?.top || 0) + 470 });
        await delay(420);
        const after = state();
      ...
      await clear();
      {
        ...
        await window.__iiimageAIDebug.fitCanvas();
        await delay(260);
        const moved = await window.__iiimageAIDebug.nativeAssetDrop({
          sourceNodeId: a,
          targetContainerId: c,
