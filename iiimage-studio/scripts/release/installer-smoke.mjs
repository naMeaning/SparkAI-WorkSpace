...
const installer = resolve(
  ...
  join(projectRoot, "release", `iiimage-Studio-Setup-${version}-x64.exe`)
);
...
const installDir = join(runDir, "中文 安装路径", "iiimage Studio");
const installedExe = join(installDir, "iiimage Studio.exe");
const uninstaller = join(installDir, "iiimage-studio-uninstaller.exe");
const failureRollbackDir = join(runDir, "故障回滚路径", "iiimage Studio");
const cancelledInstallDir = join(runDir, "取消安装路径", "iiimage Studio");
const watchdogRollbackDir = join(runDir, "Watchdog 回滚路径", "iiimage Studio");
const noShortcutDir = join(runDir, "无快捷方式路径", "iiimage Studio");
const noShortcutExe = join(noShortcutDir, "iiimage Studio.exe");
const noShortcutUninstaller = join(noShortcutDir, "iiimage-studio-uninstaller.exe");
const noShortcutRepairAttemptDir = join(runDir, "无快捷方式禁止迁移路径", "iiimage Studio");
const noShortcutRepairAttemptUninstaller = join(noShortcutRepairAttemptDir, "iiimage-studio-uninstaller.exe");
const migrationAttemptDir = join(runDir, "禁止迁移路径", "iiimage Studio");
const migrationUninstaller = join(migrationAttemptDir, "iiimage-studio-uninstaller.exe");
const lockedOptionsCapture = join(runDir, "existing-install-path-locked.png");
const preservedDataDir = join(runDir, "卸载保留数据", "iiimage Studio");
const preservedDataSentinel = join(preservedDataDir, "project-library-preserved.txt");
const preservedLocalDataDir = join(runDir, "卸载保留本地辅助数据", "iiimage Studio");
const preservedLocalDataSentinel = join(preservedLocalDataDir, "installer-log-preserved.txt");
const clearedDataDir = join(runDir, "卸载清理数据", "iiimage Studio");
const clearedDataSentinel = join(clearedDataDir, "project-library-cleared.txt");
const clearedLocalDataDir = join(runDir, "卸载清理本地辅助数据", "iiimage Studio");
...
const protectedProgramFilesDir = join(process.env.ProgramFiles || "C:\\Program Files", `iiimage Studio Smoke ${stamp}`);
const protectedWindowsDir = join(process.env.SystemRoot || "C:\\Windows", "System32", `iiimage Studio Smoke ${stamp}`);
const reportPath = join(runDir, "report.json");
const operationLockPath = join(tmpdir(), "iiimage-studio-installer", "operation-lock", "install-uninstall.lock");
...
const smokeEnvironment = {
  ...process.env,
  IIIMAGE_INSTALLER_DIAGNOSTIC_IGNORE_MACHINE: "1",
  IIIMAGE_INSTALLER_DIAGNOSTIC_ALLOW_DATA_OVERRIDE: "1",
  IIIMAGE_INSTALLER_DIAGNOSTIC_ALLOW_TIMEOUT_OVERRIDE: "1"
};
...
function shortcutPaths() {
  ...
  const desktop = desktopRoots.flatMap((root) => walkFiles(root)).filter((path) => (
    basename(path).toLowerCase() === "iiimage studio.lnk"
  ));
  const startMenu = startMenuRoot && existsSync(startMenuRoot)
    ? walkFiles(startMenuRoot).filter((path) => basename(path).toLowerCase() === "iiimage studio.lnk")
    : [];
...
function installerProcessSnapshot() {
  const script = `
$needle = $env:IIIMAGE_SMOKE_RUN_DIR
...
`;
  for (const shell of ["pwsh.exe", "powershell.exe"]) {
    const result = spawnSync(shell, ["-NoProfile", "-NonInteractive", "-Command", script], {
      ...
      encoding: "utf8",
      timeout: 30_000,
      env: { ...process.env, IIIMAGE_SMOKE_RUN_DIR: runDir }
    });
...
function signatureStatus(filePath) {
  ...
  for (const shell of ["pwsh.exe", "powershell.exe"]) {
    const result = spawnSync(shell, [
      ...
      "-NonInteractive",
      "-Command",
      "(Get-AuthenticodeSignature -LiteralPath $env:IIIMAGE_SIGNATURE_PATH).Status.ToString()"
    ], {
      cwd: projectRoot,
      windowsHide: true,
      encoding: "utf8",
      timeout: 30_000,
      env: { ...process.env, IIIMAGE_SIGNATURE_PATH: filePath }
    });
...
foreach ($view in @([Microsoft.Win32.RegistryView]::Registry64, [Microsoft.Win32.RegistryView]::Registry32)) {
  ...
  try {
    $key = $base.OpenSubKey($env:IIIMAGE_UNINSTALL_REGISTRY_PATH)
    $installKey = $base.OpenSubKey($env:IIIMAGE_INSTALL_REGISTRY_PATH)
    if ($null -ne $key) {
      try {
...
`;
  ...
  for (const shell of ["pwsh.exe", "powershell.exe"]) {
    const result = spawnSync(shell, ["-NoProfile", "-NonInteractive", "-Command", script], {
      ...
      env: {
        ...smokeEnvironment,
        IIIMAGE_UNINSTALL_REGISTRY_PATH: registryPath,
        IIIMAGE_INSTALL_REGISTRY_PATH: installRegistryPath
      }
    });
...
const preexistingInstallCandidates = uniqueExisting([
  join(process.env.LOCALAPPDATA || "", "Programs", "iiimage Studio", "iiimage Studio.exe"),
  join(process.env.LOCALAPPDATA || "", "iiimage Studio", "iiimage Studio.exe")
]);
...
if (
  ...
  throw new Error(
    "A pre-existing iiimage Studio installation, uninstall registration, or shortcut was detected; refusing to run destructive installer smoke."
  );
}
...
try {
  ...
  watchdogRollbackResult = await run(installer, ["/S", `/D=${watchdogRollbackDir}`], 120_000, {
    IIIMAGE_INSTALLER_DIAGNOSTIC_STALL_CORE: "1",
    IIIMAGE_INSTALLER_DIAGNOSTIC_CORE_TIMEOUT_MS: "120",
    IIIMAGE_INSTALLER_DIAGNOSTIC_RECOVERY_TIMEOUT_MS: "120"
  });
  ...
  installResult = await run(installer, ["/S", `/D=${installDir}`], 300_000, {
    IIIMAGE_INSTALLER_DIAGNOSTIC_CORE_TIMEOUT_MS: "1"
  });
  ...
  if (
    !brandedUninstallRegistration.toLowerCase().includes("iiimage-studio-uninstaller.exe") ||
    !String(brandedUninstallEntry?.quietUninstallString || "").toLowerCase().includes("iiimage-studio-uninstaller.exe") ||
  ...
  uninstallResult = await run(uninstaller, [
    ...
    IIIMAGE_UNINSTALLER_DIAGNOSTIC_CORE_TIMEOUT_MS: "1"
  });
