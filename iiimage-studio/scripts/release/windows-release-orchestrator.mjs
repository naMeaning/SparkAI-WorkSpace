...
const incompleteMarkerPath = join(releaseDir, ".iiimage-release-incomplete.json");
...
$item = Get-Item -LiteralPath $env:IIIMAGE_RELEASE_EXE
...
function releaseArtifactPaths(root = releaseDir, targetVersion = version) {
  const installerName = `iiimage-Studio-Setup-${targetVersion}-x64.exe`;
  const restartName = `iiimage-Studio-Restart-Update-${targetVersion}-x64.asar`;
  return {
    installerName,
    restartName,
...
export function invalidateReleaseCompleteness(root = releaseDir, targetVersion = version, marker = {}) {
  ...
  const markerPath = join(root, ".iiimage-release-incomplete.json");
  writeFileSync(markerPath, `${JSON.stringify({
    product: "iiimage-studio",
    version: targetVersion,
    incomplete: true,
    updatedAt: new Date().toISOString(),
    ...marker
  }, null, 2)}\n`, "utf8");
  for (const filePath of [artifacts.manifestPath, artifacts.sidecarPath, artifacts.checksumPath, artifacts.restartPath]) {
...
function ensureKeyPairMatches() {
  ...
  const probe = Buffer.from(`iiimage-release-preflight:${version}:${compatibility}`, "utf8");
...
$running = @(Get-Process -Name 'iiimage Studio' -ErrorAction SilentlyContinue)
[ordered]@{
...
function resolveReleaseInputs() {
  return {
    baselineExe: resolve(valueArg(
      "--baseline-exe",
      process.env.IIIMAGE_RELEASE_BASELINE_EXE || defaultBaselineExe
    )),
    settingsPath: resolve(valueArg(
      "--settings",
      process.env.IIIMAGE_RELEASE_SETTINGS || defaultSettingsPath
    ))
  };
}
...
function verifyFormalInputs(inputs) {
  ...
  if (String(baselineMetadata.productName || "") !== "iiimage Studio") {
    throw new Error("更新 E2E 基线不是 iiimage Studio。");
  }
  const baselineVersion = String(baselineMetadata.productVersion || baselineMetadata.fileVersion || "");
  if (compareVersions(baselineVersion, version) !== -1) {
    throw new Error(`更新 E2E 基线版本必须低于 ${version}，当前为 ${baselineVersion || "unknown"}。`);
  }
  ...
  if (machine.installed || Number(machine.runningCount) > 0) {
    throw new Error("检测到已安装或正在运行的 iiimage Studio；正式安装 smoke 拒绝触碰现有用户环境。");
  }
...
export function verifyReleaseArtifactsAt(root = releaseDir, targetVersion = version, options = {}) {
  ...
  if (
    ...
    manifest.product !== "iiimage-studio" ||
    manifest.channel !== "stable" ||
    manifest.version !== targetVersion ||
    manifest.compatibility !== expectedCompatibility ||
    !parseVersion(manifest.minimum_version)
  ) {
  ...
  if (!options.skipExecutableMetadata) {
    ...
    if (
      ...
      metadata.productName !== "iiimage Studio" ||
...
async function runFormalRelease() {
  ...
  process.env.IIIMAGE_RELEASE_BASELINE_EXE = inputs.baselineExe;
  process.env.IIIMAGE_RELEASE_SETTINGS = inputs.settingsPath;
  ...
  try {
    ...
    report.restartRuntimeBoundary = verifyRestartRuntimeBoundary({
...
function runSelftest() {
  ...
  const invalidationOk =
    existsSync(join(fixtureRelease, ".iiimage-release-incomplete.json")) &&
  ...
  const report = { ok: true, root, verified, invalidationOk, tamperRejected, formalReleaseStepNames };
