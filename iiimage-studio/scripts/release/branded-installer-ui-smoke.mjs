...
const uninstaller = resolve(
  ...
  join(projectRoot, ".release-tools", "brand-uninstaller", "iiimage Studio Uninstaller.exe")
);
const stamp = new Date().toISOString().replace(/[:.]/g, "-");
const outputDir = join(projectRoot, ".diagnostics", "release", `branded-installer-ui-${stamp}`);
const reportPath = join(outputDir, "report.json");
const smokeEnvironment = { ...process.env, IIIMAGE_INSTALLER_DIAGNOSTIC_IGNORE_MACHINE: "1" };
...
for (const capture of captures) {
  ...
  const environment = logicalWorkArea
    ? { ...smokeEnvironment, IIIMAGE_INSTALLER_DIAGNOSTIC_WORKAREA: `${logicalWorkArea[0]}x${logicalWorkArea[1]}` }
    : smokeEnvironment;
