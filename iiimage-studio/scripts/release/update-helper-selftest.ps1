...
$root = Join-Path ([System.IO.Path]::GetTempPath()) ("iiimage-update-helper-test-" + [Guid]::NewGuid().ToString("N"))
...
function Invoke-HelperCase([string]$Name, [bool]$Healthy) {
  ...
  if ($Healthy) {
    ...
    if (Test-Path -LiteralPath "$target.iiimage-backup") { throw "healthy update left a backup behind" }
  } else {
...
function Invoke-LauncherCase() {
  ...
  while ([DateTime]::UtcNow -lt $deadline) {
    ...
    if ($healthyLog -and (Get-Content -LiteralPath $target -Raw) -eq "new-release" -and -not (Test-Path -LiteralPath "$target.iiimage-backup")) { break }
  ...
  if (Test-Path -LiteralPath "$target.iiimage-backup") { throw "launcher update left a backup behind" }
}
...
function Invoke-ChecksumMismatchCase() {
  ...
  if (Test-Path -LiteralPath "$target.iiimage-backup") { throw "checksum mismatch left a rollback backup" }
}
