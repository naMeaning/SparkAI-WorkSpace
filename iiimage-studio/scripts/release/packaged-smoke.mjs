...
const executable = resolve(exeArg?.split("=").slice(1).join("=") || join(projectRoot, "release", "win-unpacked", "iiimage Studio.exe"));
...
const child = spawn(executable, [
  ...
  env: {
    ...process.env,
    IIIMAGE_AIDEBUG: "1",
    IIIMAGE_AIDEBUG_LIVE_IMAGE: liveNetwork ? "1" : "0",
    IIIMAGE_AIDEBUG_MOCK_AGENT: "1",
    IIIMAGE_CONFIG_DIR: configDir,
    IIIMAGE_DEBUG_DIR: debugDir,
...
try {
  ...
  const expression = `(async () => {
    ...
    const bridgeReady = await waitUntil(() => Boolean(
      window.iiimageConfig && window.iiimageAgent &&
      (!${JSON.stringify(liveNetwork)} || (window.iiimageServer && window.iiimageUpdater))
    ));
    if (!bridgeReady) throw new Error("Production preload bridges did not become ready.");
    const settings = await window.iiimageConfig.loadSettings();
    const projects = await window.iiimageConfig.listProjects();
    const smoke = await window.iiimageAgent.smoke();
    ...
    const readBack = saved?.asset?.path
      ? await window.iiimageConfig.readAssetDataUrl({ path: saved.asset.path })
      : { ok: false, error: "saved asset path missing" };
    const imported = await window.iiimageConfig.importLocalImage({
      path: ${JSON.stringify(fixturePath)},
      projectId: "default"
    });
    const semantic = await window.iiimageConfig.refineSemanticLayers({
      width: 64,
      height: 64,
    ...
    const psd = saved?.asset
      ? await window.iiimageConfig.exportAssetPsd({
          asset: saved.asset,
          assetIndex: 0,
    ...
    return {
      ok: Boolean(
        ...
        title === "iiimage Studio" && bodyText.length > 20
      ),
      title,
      bodyTextLength: bodyText.length,
      bridges: {
        config: Object.keys(window.iiimageConfig || {}),
        agent: Object.keys(window.iiimageAgent || {}),
        server: Object.keys(window.iiimageServer || {})
      },
