...
const desktopLog = desktopLogArg?.split("=").slice(1).join("=") ||
  join(defaultDesktopLogDir, `IIimage-AIDebug-人类化真实输入-${runId}.md`);
...
const fallbackViewFixturePath = join(repoRoot, "public", "iiimage-studio.png");
...
async function screenshot(client, label) {
  ...
  try {
    ...
    for (const scope of ["page", "window"]) {
      try {
        const nativeCapture = await evaluate(
          client,
          `window.__iiimageDebugCaptureGui ? window.__iiimageDebugCaptureGui(${JSON.stringify(label)}, ${JSON.stringify(scope)}) : Promise.resolve({ ok: false, error: "capture hook missing" })`,
          20000
        );
    ...
    let debugState = null;
    try {
      debugState = await evaluate(client, "window.__iiimageDebugAgentState?.() || null", 8000);
    } catch {
      debugState = null;
    }
...
function writeScreenshotContactSheet(screenshots) {
  ...
  writeFileSync(filePath, [
    "<!doctype html>",
    "<meta charset=\"utf-8\">",
    "<title>IIimage AIDebug Screenshot Contact Sheet</title>",
    "<style>",
    ...
    "</style>",
    "<h1>IIimage AIDebug Screenshot Contact Sheet</h1>",
    `<p>runDir: ${runDir}</p>`,
    "<div class=\"grid\">",
    cards,
    "</div>"
  ...
  try {
    if (statusRequested) {
      const state = await evaluate(
        client,
        `(() => {
          const debugState = window.__iiimageDebugAgentState?.() || {};
        ...
        stepResult.cleanup = await evaluate(
          ...
          `new Promise((resolve) => {
            ...
            const timer = window.setInterval(() => {
              const status = String(window.__iiimageDebugAgentState?.()?.agentStatus || "");
    ...
    if (suiteName === "tools") {
      ...
      await runComposerStep(
        7,
        "web-search",
        "请用 web_search 查一下 IIimage Agent 工具 schema 设计这个关键词，不要生图。",
    ...
    if (suiteName === "tool-natural") {
      const isolation = await evaluate(
        client,
        `Promise.resolve(window.__iiimageAIDebug?.newConversation?.()).then((result) => ({
    ...
    if (suiteName === "context-compact-tools") {
      ...
      const setup = await evaluate(
        ...
        `new Promise((resolve) => {
          window.__iiimageDebugOpenSurface?.("main");
          Promise.resolve(window.__iiimageAIDebug?.newConversation?.()).then((freshConversation) => {
            ...
            for (let index = 0; index < messages.length; index += 12) {
              ok = Boolean(window.__iiimageDebugSeedAgentMessages?.({
                messages: messages.slice(index, index + 12),
                append: index > 0
              })) && ok;
            }
            const cleared = window.__iiimageDebugApplyAgentActions?.([{ type: "workflow.canvas.clear", mode: "all" }]);
            window.setTimeout(() => {
          ...
          }).catch((error) => resolve({ ok: false, error: String(error), state: window.__iiimageDebugAgentState?.() }));
        })`,
        30000
      );
      ...
      const setup = await evaluate(
        ...
        `new Promise((resolve) => {
          ...
          Promise.resolve(window.__iiimageAIDebug?.newConversation?.()).then((freshConversation) => {
            ...
            let seeded = true;
            for (let index = 0; index < messages.length; index += 12) {
              seeded = Boolean(window.__iiimageDebugSeedAgentMessages?.({
                messages: messages.slice(index, index + 12),
                append: index > 0
              })) && seeded;
            }
            const applied = window.__iiimageDebugApplyAgentActions?.([
            ...
            window.setTimeout(async () => {
              const selected = await window.__iiimageAIDebug?.selectNode?.({ id: "A" }).catch((error) => ({ ok: false, error: String(error) }));
              const state = window.__iiimageDebugAgentState?.() || {};
              resolve({
    ...
    if (suiteName === "ask-user-edge") {
      ...
      const setup = await evaluate(
        ...
        `new Promise((resolve) => {
          window.__iiimageDebugOpenSurface?.("main");
          const ok = window.__iiimageDebugApplyAgentActions?.([
            { type: "workflow.canvas.clear", mode: "all" },
            {
              type: "workflow.node.create",
          ...
          window.setTimeout(async () => {
            const selected = await window.__iiimageAIDebug?.selectNode?.({ id: "A" }).catch((error) => ({ ok: false, error: String(error) }));
            resolve({ ok: Boolean(ok), selected, state: window.__iiimageDebugAgentState?.() });
          }, 700);
        })`,
        20000
      );
    ...
    if (suiteName === "workflow-broad") {
      ...
      const setup = await evaluate(
        ...
        `(async () => {
          window.__iiimageDebugOpenSurface?.("main");
          const seeded = await window.__iiimageAIDebug?.seedCanvas?.({ count: 2 });
          const selected = await window.__iiimageAIDebug?.selectNode?.({ id: "A" }).catch((error) => ({ ok: false, error: String(error) }));
          const state = window.__iiimageDebugAgentState?.();
    ...
    if (suiteName === "feedback-continuation") {
      ...
      const setup = await evaluate(
        ...
        `new Promise((resolve) => {
          ...
          window.setTimeout(async () => {
            const selected = await window.__iiimageAIDebug?.selectNode?.({ id: "A" }).catch((error) => ({ ok: false, error: String(error) }));
            resolve({ ok: Boolean(ok && freshConversation?.ok !== false && freshConversation?.fastMemoryEmpty === true), freshConversation, selected, state: window.__iiimageDebugAgentState?.() });
          }, 700);
          }).catch((error) => resolve({ ok: false, error: String(error), state: window.__iiimageDebugAgentState?.() }));
        })`,
        20000
    ...
    if (suiteName === "ambiguous-intent") {
      ...
      await evaluate(client, `window.__iiimageAIDebug?.selectNode?.({ id: ${JSON.stringify(generatedNodeId)} })`, 20000);
      await runComposerStep(
        2,
    ...
    if (suiteName === "experience-boundary") {
      ...
      const setup = await evaluate(
        ...
        `new Promise((resolve) => {
          ...
          window.setTimeout(() => resolve({ ok: Boolean(ok), state: window.__iiimageDebugAgentState?.() }), 500);
        })`,
        20000
      );
