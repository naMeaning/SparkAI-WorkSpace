...
async function waitForDebugTarget(port) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      const target = targets.find((item) => item.type === "page" && (String(item.url).startsWith("file:") || String(item.title).includes("IIimage")));
...
const interactionExpression = `(async () => {
  ...
  const finalSnapshot = window.__iiimagePerformanceProbe.snapshot();
  return {
...
async function runRound(round) {
  ...
  const child = spawn(process.execPath, [electronCli, `--remote-debugging-port=${debugPort}`, `--user-data-dir=${userDataDir}`, "electron-main.cjs"], {
    ...
    env: {
      ...process.env,
      IIIMAGE_RENDERER_INDEX: join(bundleDir, "index.html"),
      IIIMAGE_AIDEBUG: "1",
      IIIMAGE_AIDEBUG_MOCK_AGENT: "1",
      IIIMAGE_AIDEBUG_AGENT_MODE: "mock",
      IIIMAGE_PERFORMANCE_GATE: "1",
  ...
  try {
    ...
    const ready = await waitForExpression(client, `window.__iiimagePerformanceProbe?.profile === 'production-like' && window.__iiimagePerformanceProbe.snapshot().rendererReady === true`);
    ...
    const startupSnapshot = await client.evaluate(`window.__iiimagePerformanceProbe.snapshot()`);
    ...
    await delay(520);
    const finalSnapshot = await client.evaluate(`window.__iiimagePerformanceProbe.snapshot()`);
    const heap = await client.send("Runtime.getHeapUsage");
    const screenshot = await client.send("Page.captureScreenshot", { format: "png", captureBeyondViewport: false });
    writeFileSync(screenshotPath, Buffer.from(screenshot.data, "base64"));
    await client.evaluate(`window.iiimageConfig?.windowControl?.({ action: 'close' })`);
...
async function main() {
  ...
  const forbidden = ["__iiimageAIDebug", "runLayerStackSuite", "runMixedStressSuite", "__iiimageDebugSendAgentPrompt"].filter((token) => sourceText.includes(token));
  const probePresent = sourceText.includes("__iiimagePerformanceProbe");
