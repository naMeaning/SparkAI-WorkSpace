...
const temporaryRoot = mkdtempSync(path.join(os.tmpdir(), "iiimage-update-selftest-"));
...
process.env.IIIMAGE_UPDATE_PUBLIC_KEY = publicKeyPath;
process.env.IIIMAGE_CONFIG_DIR = temporaryRoot;
...
try {
  const release = {
    ...
    compatibility: packageMetadata.iiimageUpdateCompatibility,
    notes: ["在线更新闭环。"],
    restart: {
      filename: `iiimage-Studio-Restart-Update-${futureReleaseVersion}-x64.asar`,
      sha256: "a".repeat(64),
      size: 1024
    },
    installer: {
      filename: `iiimage-Studio-Setup-${futureReleaseVersion}-x64.exe`,
      sha256: "b".repeat(64),
      size: 2048
    }
  };
  ...
  const incompatibleRelease = {
    ...release,
    version: "99.0.1",
    compatibility: `${packageMetadata.iiimageUpdateCompatibility}-different-shell`
  };
  ...
  const replayedCurrentRelease = {
    ...
    restart: { ...release.restart, filename: `iiimage-Studio-Restart-Update-${packageVersion}-x64.asar` },
    installer: { ...release.installer, filename: `iiimage-Studio-Setup-${packageVersion}-x64.exe` }
  };
  ...
  const updateAssetPath = path.join(updateAssetDir, "iiimage-test-update.asar");
  ...
  const staleInstallerPath = path.join(staleInstallerDir, `iiimage-Studio-Setup-${packageVersion}-x64.exe`);
