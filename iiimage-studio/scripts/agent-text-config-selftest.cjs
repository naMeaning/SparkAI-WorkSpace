...
async function runSelftest(directory) {
  ...
  try {
    ...
    assert.equal(Object.prototype.hasOwnProperty.call(viewImageSchema?.function?.parameters?.properties || {}, "brief"), false, "Codex-native view_image schema must not gain IIimage-only fields");
    ...
    assert.equal(Object.prototype.hasOwnProperty.call(shellCommandSchema.function.parameters.properties || {}, "brief"), false, "Codex-native shell_command schema must not gain IIimage-only fields");
...
function launchSelftestWorker() {
  const directory = mkdtempSync(path.join(os.tmpdir(), "iiimage-agent-text-config-selftest-"));
