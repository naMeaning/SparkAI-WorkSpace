...
export async function captureRequirementNodeSuite(context) {
  ...
  await waitForExpression(
    client,
    "Boolean(window.__iiimageAIDebug?.openRequirementForSource && window.__iiimageAIDebug?.openRequirementEditor && window.__iiimageAIDebug?.executeRequirement && window.__iiimageAIDebug?.disconnectNodeInput && window.__iiimageDebugAgentState)",
    10000
  );
  const results = [];
  ...
  const setup = await evaluate(client, `(async () => {
    ...
    let state = window.__iiimageDebugAgentState?.() || {};
    const imageIds = (state.nodes || []).filter((node) => node.type === "image" && Number(node.assetCount || 0) > 0).map((node) => node.id).slice(-2);
    if (imageIds[0]) window.__iiimageDebugMoveNode?.({ id: imageIds[0], x: 120, y: 120 });
    if (imageIds[1]) window.__iiimageDebugMoveNode?.({ id: imageIds[1], x: 800, y: 520 });
  ...
  const openFromContextMenu = await evaluate(client, `(async () => {
    const menu = await window.__iiimageAIDebug.openNodeMenu({ id: ${JSON.stringify(firstId)} });
    ...
    while (performance.now() < deadline && !window.__iiimageDebugAgentState?.().requirementEditorOpen) await new Promise((resolve) => setTimeout(resolve, 60));
    return {
      ok: Boolean(menu?.ok && button && window.__iiimageDebugAgentState?.().requirementEditorOpen),
      menu,
  ...
  results.push(await captureState(client, targetId, "requirement-node-created", "window.__iiimageAIDebug?.fitCanvas?.()", { width: 1280, height: 820 }, {
    modalOpen: false,
  ...
  const openedByDoubleClick = await evaluate(client, `(async () => {
    ...
    const deadline = performance.now() + 3000;
    while (performance.now() < deadline && !window.__iiimageDebugAgentState?.().requirementEditorOpen) await new Promise((resolve) => setTimeout(resolve, 50));
    const state = window.__iiimageDebugAgentState?.();
  ...
  const edited = await evaluate(client, `(async () => {
    ...
    if (!textarea) return { ok: false, error: "requirement editor textarea unavailable", state: window.__iiimageDebugAgentState?.() };
    ...
    return { ok: false, state: window.__iiimageDebugAgentState?.() };
  })()`);
  ...
  results.push(await captureState(client, targetId, "requirement-node-first-output", "window.__iiimageAIDebug?.fitCanvas?.()", { width: 1280, height: 820 }, {
    modalOpen: false,
    requirementNodeCount: 1,
    requirementGraphValid: true,
    requirementConnectionPortsVisible: true,
  ...
  const disconnected = await evaluate(client, `(async () => {
    ...
    const deadline = performance.now() + 3000;
    while (performance.now() < deadline) {
      const node = window.__iiimageDebugAgentState?.().nodes?.find((item) => item.id === ${JSON.stringify(requirementId)});
  ...
  const disconnectUndo = await evaluate(client, `(async () => {
    ...
    const restored = window.__iiimageDebugAgentState?.().nodes?.find((item) => item.id === ${JSON.stringify(requirementId)});
    await new Promise((resolve) => setTimeout(resolve, 120));
    document.querySelector('.flow-node[data-node-id=${JSON.stringify(requirementId)}] .node-port-in')?.click();
    ...
    return { ok: false, restored, state: window.__iiimageDebugAgentState?.() };
  })()`);
  ...
  const sameContentReconnectConfirm = await evaluate(client, `(async () => {
    const accepted = await window.__iiimageAIDebug.executeRequirement({ id: ${JSON.stringify(requirementId)} });
    const deadline = performance.now() + 3000;
    while (performance.now() < deadline && !document.querySelector(".confirm-dialog")) await new Promise((resolve) => setTimeout(resolve, 50));
    const dialog = document.querySelector(".confirm-dialog");
    return {
  ...
  const distinctSourceImport = await evaluate(client, `(async () => {
    ...
    const imported = await window.__iiimageDebugImportPathsToCanvas({
  ...
  if (distinctSourceId) {
    await waitForExpression(
      client,
      `Boolean(window.__iiimageDebugAgentState?.().nodes?.some((node) => node.id === ${JSON.stringify(distinctSourceId)} && node.imageContainer))`,
      10000
    );
  }
  ...
  const secondExecution = await evaluate(client, `(async () => {
    ...
    return { ok: false, accepted, state: window.__iiimageDebugAgentState?.() };
  })()`, 60000);
  results.push(await captureState(client, targetId, "requirement-node-reused-source", "window.__iiimageAIDebug?.fitCanvas?.()", { width: 1280, height: 820 }, {
    modalOpen: false,
    requirementNodeCount: 1,
    requirementGraphValid: true,
    requirementConnectionPortsVisible: true,
  ...
  if (containerId) {
    await waitForExpression(client, `Boolean(window.__iiimageDebugAgentState?.().nodes?.some((node) => node.id === ${JSON.stringify(containerId)} && node.imageContainer))`, 10000);
    await evaluate(client, `window.__iiimageAIDebug.openRequirementForSource({ id: ${JSON.stringify(containerId)} })`);
    await waitForExpression(client, "Boolean(window.__iiimageDebugAgentState?.().requirementEditorOpen)", 4000);
  }
  ...
  const containerRequirement = containerId ? await evaluate(client, `(async () => {
    ...
    while (performance.now() < createdDeadline) {
      const requirement = window.__iiimageDebugAgentState?.().nodes?.find((node) => node.type === "requirement" && node.parentId === ${JSON.stringify(containerId)} && node.requirement?.createdFrom === "container");
      if (requirement) return { ok: true, containerId: ${JSON.stringify(containerId)}, requirement };
      await new Promise((resolve) => setTimeout(resolve, 60));
    }
    return { ok: false, containerId: ${JSON.stringify(containerId)}, import: ${JSON.stringify(containerImport)}, state: window.__iiimageDebugAgentState?.() };
  ...
  await evaluate(client, `(() => {
    window.__iiimageRequirementLayerProbe = { status: "running", result: null };
    Promise.resolve().then(async () => {
      const layers = await window.__iiimageAIDebug.runLayerStackSuite({ agentDriven: false });
      const sourceId = String(layers?.subjectId || layers?.layerNodeIds?.[0] || "");
      if (!layers?.ok || !sourceId) {
        window.__iiimageRequirementLayerProbe = {
          status: "complete",
          result: { ok: false, sourceId, layers: { ok: Boolean(layers?.ok), groupId: layers?.groupId || "", layerNodeIds: layers?.layerNodeIds || [] } }
        };
        return;
      }
      await window.__iiimageAIDebug.openRequirementForSource({ id: sourceId });
      const textarea = document.querySelector(".requirement-editor-dialog .requirement-text-field textarea");
      ...
      const deadline = performance.now() + 4000;
      while (performance.now() < deadline) {
        const requirement = window.__iiimageDebugAgentState?.().nodes?.find((node) => node.type === "requirement" && node.parentId === sourceId && node.requirement?.createdFrom === "layer");
        if (requirement) {
          window.__iiimageRequirementLayerProbe = {
            status: "complete",
            result: {
  ...
  await waitForExpression(client, "window.__iiimageRequirementLayerProbe?.status !== 'running'", 120000);
  phase("capture-layer-requirement");
  const layerRequirement = await evaluate(client, "window.__iiimageRequirementLayerProbe?.result || { ok: false, error: 'layer requirement probe missing' }");
  ...
  const round2Scope = await evaluate(client, `(async () => {
    ...
    const sourceCreated = await window.__iiimageDebugCreateImageContainer?.({ worldX: 100, worldY: 130, role: "source" });
    const referenceCreated = await window.__iiimageDebugCreateImageContainer?.({ worldX: 100, worldY: 470, role: "reference" });
    const neutralCreated = await window.__iiimageDebugCreateImageContainer?.({ worldX: 760, worldY: 470 });
    ...
    const sourceImport = await window.__iiimageDebugImportPathsToCanvas?.({ paths: [${JSON.stringify(fixturePaths[0])}], targetContainerId: sourceId });
    const referenceImport = await window.__iiimageDebugImportPathsToCanvas?.({ paths: [${JSON.stringify(fixturePaths[1] || fixturePaths[0])}], targetContainerId: referenceId });
    const neutralImport = await window.__iiimageDebugImportPathsToCanvas?.({ paths: [${JSON.stringify(fixturePaths[2] || fixturePaths[0])}], targetContainerId: neutralId });
    ...
    const neutralAfterConversion = window.__iiimageDebugAgentState?.().nodes?.find((node) => node.id === neutralId);
    ...
    while (performance.now() < createDeadline) {
      requirementNode = window.__iiimageDebugAgentState?.().nodes?.find((node) => node.type === "requirement" && node.requirement?.createdFrom === "canvas");
      if (requirementNode && !window.__iiimageDebugAgentState?.().requirementEditorOpen) break;
      await delay(60);
    }
    ...
    const sourceConnected = window.__iiimageDebugConnectNodes?.({ sourceId, targetId: requirementId }) === true;
    const referenceConnected = window.__iiimageDebugConnectNodes?.({ sourceId: referenceId, targetId: requirementId }) === true;
    await delay(240);
    requirementNode = window.__iiimageDebugAgentState?.().nodes?.find((node) => node.id === requirementId);
    ...
    while (performance.now() < scopeDeadline) {
      taskScope = window.__iiimageDebugAgentState?.().lastDispatchedTaskScope || null;
    ...
    while (performance.now() < finishDeadline && window.__iiimageDebugAgentState?.().agentStatus !== "idle") await delay(120);
    const state = window.__iiimageDebugAgentState?.() || {};
  ...
  const finalCapture = await captureState(client, targetId, "requirement-node-role-scope", "window.__iiimageAIDebug?.fitCanvas?.()", { width: 1280, height: 820 }, {
    modalOpen: false,
    requirementNodeCount: 1,
    requirementGraphValid: true,
    requirementConnectionPortsVisible: true,
