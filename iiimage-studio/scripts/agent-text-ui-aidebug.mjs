...
const requestedDebugPort = Number(portArg?.split("=")[1] || process.env.IIIMAGE_REMOTE_DEBUGGING_PORT || 0);
let debugPort = 0;
const electronCli = [
...
function startElectron() {
  ...
  return spawn(command, commandArgs, {
    ...
    env: {
      ...
      IIIMAGE_AIDEBUG: "1",
      IIIMAGE_AIDEBUG_LIVE_IMAGE: "0",
      IIIMAGE_AIDEBUG_REAL_AGENT: "0",
      IIIMAGE_AIDEBUG_MOCK_AGENT: "1",
      IIIMAGE_AIDEBUG_AGENT_MODE: "mock",
...
async function openPromptEditor(client) {
  await evaluate(client, `window.__iiimageDebugOpenSurface?.("settings")`);
...
async function main() {
  ...
  let fatalError = "";
  try {
    await waitForExpression(client, `Boolean(document.querySelector('.ide-shell') && window.iiimageAgent && window.__iiimageDebugAgentState)`);
    await setWindowSize(client, target.id, 1280, 820);

    const toolsBefore = await evaluate(client, `window.iiimageAgent.tools()`);
    ...
    await evaluate(client, `window.__iiimageDebugSurfaceSaveDelayMs = 1200`);
    ...
    await pressKey(client, "Escape");
    await delay(100);
    const accountBusyEscape = await evaluate(client, `({ present: Boolean(document.querySelector('.account-drawer')), trace: window.__iiimageDebugLastSurfaceClose || null })`);
    ...
    await delay(100);
    const accountBusyBackdrop = await evaluate(client, `({ present: Boolean(document.querySelector('.account-drawer')), trace: window.__iiimageDebugLastSurfaceClose || null })`);
    ...
    await evaluate(client, `window.__iiimageDebugSurfaceSaveDelayMs = 0`);
    ...
    const savedPrompt = await evaluate(client, `window.iiimageAgent.getMainPrompt()`);
    ...
    const toolsAfter = await evaluate(client, `window.iiimageAgent.tools()`);
    ...
    await evaluate(client, `(() => {
      ...
      textarea.__iiimagePromptResizeIdentity = ${JSON.stringify(resizeIdentity)};
      return true;
    })()`);
    await setWindowSize(client, target.id, 884, 720);
    ...
    const promptResizeContinuity = await evaluate(client, `(() => {
      ...
      return {
        ...
        identity: textarea?.__iiimagePromptResizeIdentity || '',
    ...
    await evaluate(client, `window.__iiimageDebugSurfaceSaveDelayMs = 2000`);
    await clickButton(client, "保存", ".agent-text-editor-dialog");
    ...
    const busyEscapeTrace = await evaluate(client, `window.__iiimageDebugLastSurfaceClose || null`);
    ...
    const busyBackdropTrace = await evaluate(client, `window.__iiimageDebugLastSurfaceClose || null`);
    ...
    const resetPrompt = await evaluate(client, `window.iiimageAgent.getMainPrompt()`);
    ...
    await setWindowSize(client, target.id, 1280, 820);
    await evaluate(client, `window.__iiimageDebugOpenSurface?.('agent-timeline')`);
    await waitForExpression(client, `window.__iiimageDebugAgentState?.().messageCount >= 2`);
    ...
    const firstScope = await evaluate(client, `window.__iiimageDebugAgentState()`);
    ...
    const memoryA = await evaluate(client, `window.iiimageAgent.getFastMemory(${JSON.stringify(firstScopeInput)})`);
    ...
    await evaluate(client, `window.__iiimageDebugOpenSurface?.('agent-timeline')`);
    await waitForExpression(client, `window.__iiimageDebugAgentState?.().messageCount >= 2`);
    const conversationABeforeNew = await evaluate(client, `window.__iiimageDebugAgentState?.().activeConversationId`);
    ...
    await waitForExpression(client, `window.__iiimageDebugAgentState?.().activeConversationId !== ${JSON.stringify(firstScopeInput.conversationId)}`);
    const secondScope = await evaluate(client, `window.__iiimageDebugAgentState()`);
    ...
    const clearedMemoryA = await evaluate(client, `window.iiimageAgent.getFastMemory(${JSON.stringify(firstScopeInput)})`);
    const preservedMemoryB = await evaluate(client, `window.iiimageAgent.getFastMemory(${JSON.stringify(secondScopeInput)})`);
    ...
    await evaluate(client, `new Promise((resolve) => {
      ...
      setTimeout(() => {
        window.__iiimageDebugConnectNodes?.({ sourceId: 'motion-node-a', targetId: 'motion-node-b' });
    ...
    await evaluate(client, `window.__iiimageDebugOpenSurface?.('agent-image-running')`);
    ...
    await evaluate(client, `window.__iiimageDebugApplyAgentActions?.([
      {
    ...
    await pressKey(client, "Escape");
    await waitForExpression(client, `!document.querySelector('.agent-text-editor-dialog')`);
    await evaluate(client, `window.__iiimageDebugApplyAgentActions?.([{ type: 'workflow.canvas.clear', mode: 'all' }])`);
    await evaluate(client, `window.__iiimageDebugOpenSurface?.('main')`);
    ...
    const requirementSeed = await evaluate(client, `window.__iiimageAIDebug?.seedCanvas?.({ count: 1, fileBacked: true })`);
    ...
    const requirementSourceMenu = await evaluate(client, `window.__iiimageAIDebug?.openNodeMenu?.({ id: ${JSON.stringify(requirementSourceId)} })`);
    check(
    ...
    await waitForExpression(client, `!document.querySelector('.requirement-editor-dialog') && window.__iiimageDebugAgentState?.()?.nodes?.some((node) => node.id === ${JSON.stringify(requirementNodeId)} && node.requirement?.revision === 2)`);
    const revisedRequirement = await evaluate(client, `window.__iiimageDebugAgentState?.()?.nodes?.find((node) => node.id === ${JSON.stringify(requirementNodeId)})`);
    ...
    await evaluate(client, `window.__iiimageAIDebug?.openNodeMenu?.({ id: ${JSON.stringify(requirementNodeId)} })`);
    ...
    await waitForExpression(client, `(() => {
      const node = window.__iiimageDebugAgentState?.()?.nodes?.find((item) => item.id === ${JSON.stringify(requirementNodeId)});
      return !node?.parentId && !(node?.requirement?.inputBindings?.length);
    })()`);
    const reconnected = await evaluate(client, `window.__iiimageDebugConnectNodes?.({ sourceId: ${JSON.stringify(requirementSourceId)}, targetId: ${JSON.stringify(requirementNodeId)} })`);
    check("requirement input can be disconnected and reconnected to a source", reconnected === true, { reconnected });
    const requirementPreRunState = await evaluate(client, `window.__iiimageDebugAgentState?.()`);
    ...
    await evaluate(client, `window.__iiimageAIDebug?.selectNodes?.({ ids: [${JSON.stringify(requirementNodeId)}], primaryId: ${JSON.stringify(requirementNodeId)} })`);
    await evaluate(client, `window.__iiimageDebugSendAgentPrompt?.(${JSON.stringify(requirementSelectionPrompt)})`);
    await waitForExpression(client, `window.__iiimageDebugAgentState?.()?.agentStatus === 'idle' && window.__iiimageDebugAgentState?.()?.messages?.some((message) => message.role === 'user' && message.content === ${JSON.stringify(requirementSelectionPrompt)})`, 15000);
    ...
    const followSeeded = await evaluate(client, `window.__iiimageDebugSeedAgentMessages?.({
    ...
    await evaluate(client, `window.__iiimageDebugSeedAgentMessages?.({ streamDelta: { id: 'follow-assistant-59', delta: '\\n实时增量不会抢走阅读位置。', status: 'running' } })`);
    ...
    await evaluate(client, `window.__iiimageDebugSeedAgentMessages?.({
    ...
    await evaluate(client, `window.__iiimageDebugSeedAgentMessages?.({ clear: true })`);

    const markdownSeeded = await evaluate(client, `window.__iiimageDebugSeedAgentMessages?.({
      messages: [
