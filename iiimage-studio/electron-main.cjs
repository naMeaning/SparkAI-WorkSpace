...
const { closeSync, copyFileSync, createReadStream, createWriteStream, existsSync, fstatSync, lstatSync, mkdirSync, openSync, readSync, readdirSync, readFileSync, realpathSync, renameSync, rmSync, statfsSync, statSync, writeFileSync } = require("node:fs");
const {
...
const updateCompatibility = String(packageMetadata.iiimageUpdateCompatibility || "").trim();
const updatePublicKeyPath = process.env.IIIMAGE_UPDATE_PUBLIC_KEY
  ? path.resolve(process.env.IIIMAGE_UPDATE_PUBLIC_KEY)
...
const updateHealthToken = String(process.argv.find((item) => item.startsWith("--iiimage-update-token=")) || "").split("=").slice(1).join("=").trim();
const updateRollbackDetected = process.argv.includes("--update-rollback") || process.env.IIIMAGE_UPDATE_ROLLBACK_SELFTEST === "1";
...
async function serverChatCompletion(payload = {}) {
  if (aidebugMode && aidebugMockAgent) {
    const startedAt = Date.now();
    ...
    const result = aidebugChatCompletion(payload);
...
async function newApiCurlTransportFetch(target, payload, options = {}) {
  ...
  const configLines = [
    ...
    `user-agent = "${newApiCurlConfigValue(`${applicationName}/${currentDesktopVersion()}`)}"`
  ];
...
function verifyDesktopReleasePayload(value) {
  ...
  if (release.schema_version !== 1 || release.product !== "iiimage-studio" || !parseDesktopVersion(version)) {
...
function desktopUpdateRollbackRecovery(pending = null) {
  ...
  if (persistedDesktopUpdateRollback?.active && persistedDesktopUpdateRollback.version && !updateRollbackDetected) {
    ...
    if (recoveredVersionComparison !== null && recoveredVersionComparison >= 0) {
      ...
      latestDesktopUpdateProgress = normalizeDesktopUpdateProgress({
        stage: "checked",
        message: `已通过完整安装更新到 iiimage Studio ${currentDesktopVersion()}。`,
...
function effectiveDesktopUpdateProgress(pending) {
  ...
  if (["checking", "downloading", "retrying", "verifying", "applying"].includes(progress.stage)) {
    if (comparison !== null && comparison >= 0) {
      return normalizeDesktopUpdateProgress({
        ...progress,
        stage: "checked",
        message: `已更新到 iiimage Studio ${currentDesktopVersion()}。`,
        percent: 100
      });
    }
    return normalizeDesktopUpdateProgress({
  ...
  if (progress.stage === "ready" && !pending) {
    return comparison !== null && comparison >= 0
      ? normalizeDesktopUpdateProgress({ ...progress, stage: "checked", message: `已更新到 iiimage Studio ${currentDesktopVersion()}。`, percent: 100 })
...
function publishDesktopUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    if (!window.isDestroyed() && !window.webContents.isDestroyed()) {
      window.webContents.send("iiimage:update:progress", message);
    }
  }
}
...
async function performDesktopUpdateCheck() {
  ...
  publishDesktopUpdateProgress({
    ...
    message: recovery && verified.updateAvailable
      ? `已切换为 iiimage Studio ${verified.latestVersion} 完整安装恢复。`
      : verified.updateAvailable
        ? `发现 iiimage Studio ${verified.latestVersion}`
        : "当前已是最新版本。",
...
async function downloadDesktopArtifact(settings, downloadUrl, artifact, options = {}) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    try {
      const headers = { ...newApiUserAuthHeaders(settings) };
...
async function applyDesktopRestartUpdate() {
  ...
  publishDesktopUpdateProgress({ stage: "applying", message: "iiimage Studio 即将重启并完成更新…", version: pending.version, kind: pending.kind });
...
async function launchDesktopInstallerUpdate() {
  return serializeDesktopUpdateApplication("installer", async () => {
    ...
    desktopUpdateHandoffAccepted = true;
    publishDesktopUpdateProgress({ stage: "applying", message: "新版安装器已启动，iiimage Studio 正在安全退出…", version: pending.version, kind: pending.kind });
...
if (projectIoSelftestMode || agentProtocolSelftestMode) {
  ...
  app.whenReady().then(() => {
    ...
    ensureRuntimeFiles();
    desktopUpdateRollbackRecovery(publicPendingDesktopUpdate());
