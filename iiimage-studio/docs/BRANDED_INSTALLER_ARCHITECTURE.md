# iiimage Studio 品牌安装与卸载架构

## 交付目标

Windows 用户从双击安装包到安装完成，全程只看到 iiimage Studio 自有界面；从“已安装的应用”、开始菜单或内部卸载入口发起卸载时，也只看到同一视觉体系。electron-builder 生成的旧式 MUI 向导不再作为公开界面。
...
界面采用深色东方科技、青绿色主色和正式 Windows 图标。所有按钮、间距、文字层级和错误反馈来自同一个组件层，不依赖 Windows 默认向导控件。

公开安装器与卸载器都以单一 EXE 运行，不依赖旁置 `.config` 文件。Windows 文件属性统一显示 `iiimage Studio`、`Aieyra` 和纯净的发布版本号，不把内部 Git 提交哈希暴露成用户可见产品版本；提交身份仍由 Forge 版本记录和发布清单追踪。

品牌壳以 920 × 620 设计坐标构建，并根据当前显示器可用工作区等比缩放；文字、图形与控件仍由 WPF 矢量渲染。这样在 1366 × 768 且 125%、150%、200% 缩放的环境下不会越出屏幕，也不会为了适配高 DPI 改回系统向导。

品牌壳目标为 `.NET Framework 4.8`、Windows x64。受支持且保持系统更新的 Windows 10/11 已具备该运行时，因此不需要把约 78 MB 的独立 Desktop Runtime 重复塞入安装包。这样最终安装包仍约 113 MB，而 self-contained WPF 方案会膨胀到约 263 MB。若未来需要支持缺少 .NET Framework 4.8 的裁剪系统，应另行提供运行时预检 bootstrapper，不能静默回退到旧式安装界面。
...
公开安装包将 NSIS 安装器作为经过 SHA-256 校验的嵌入资源。品牌安装器仅用 `/S` 调用它，所以不会创建或闪现 MUI 页面。`build/installer.nsh` 将 Windows 卸载注册改为 `iiimage-studio-uninstaller.exe`；内部 `Uninstall iiimage Studio.exe` 只作为静默删除内核保留。即使用户直接双击内部卸载器，也会在页面创建前转交给品牌卸载器。
...
5. 用品牌安装器覆盖 `release/iiimage-Studio-Setup-<version>-x64.exe`。
...
`pnpm run release:manifest` 会分别记录完整安装包和重启更新 ASAR 的大小与 SHA-256，使用项目内发布私钥签署规范化清单，并立即用随客户端打包的 `build/update-public-key.pem` 反向验签。私钥与客户端公钥不匹配时发布会直接失败，不能生成一个客户端必然拒绝的更新清单。完成后同时生成 `SHA256SUMS.txt`，覆盖安装包、重启 ASAR、签名清单和安装包旁置元数据，供 Forge 下载页与人工核验统一使用。
...
编排开始前会写入 `release/.iiimage-release-incomplete.json` 并清除当前版本旧 manifest、sidecar、Restart ASAR 与校验单。任一步失败、源工作树在执行期间变化或进程收到中断信号，都会再次撤销这些“完整发布”标志并保留失败标记；只有最后一项验签与 SHA 门禁通过才移除标记。更新 E2E 基线通过 `IIIMAGE_RELEASE_BASELINE_EXE` 指定，必须位于项目目录内、版本低于当前 RC，且包含 update helper；编排器会复制后再测试，不修改基线本体。
