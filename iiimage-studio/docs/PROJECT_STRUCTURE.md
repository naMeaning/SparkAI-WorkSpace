...
本仓库只维护 Windows Electron 客户端与本地单 Agent runtime。服务端 CRM、New API 与下载站属于独立 `ai-native` 仓库，不复制到这里。符号级所有权、进程调用链与测试影响见 [CONTEXT_MAP.md](./CONTEXT_MAP.md)。
...
| `tools/dev-launcher/` | 开发快捷启动器源码 | `iiimage Studio Dev.exe` 由这里构建。 |
| `tools/windows-installer/` | 品牌安装器与卸载器源码 | WPF 自绘交互层；NSIS 作为用户不可见的静默安装/删除内核。 |
| `assets/` | 人工维护的源素材 | Windows 图标权威源文件为 `assets/windows/iiimage-studio.ico`。 |
