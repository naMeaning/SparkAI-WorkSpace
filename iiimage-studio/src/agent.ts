...
- It may call window.iiimageAgent and normalize Agent runtime results.
...
export async function requestAgent(
  ...
  if (!window.iiimageAgent) {
    throw new Error("Agent 桥接不可用，请重启桌面端。");
  }

  const result = await window.iiimageAgent.chat({ runId, prompt, messages, nodes, referenceImages, taskScope, projectId, conversationId, selectedNodeId, selectedNodeIds });
  if (!result.ok) {
