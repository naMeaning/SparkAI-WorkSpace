...
function AuthTitlebar({ status }: { status: string }) {
  return (
    <header className="ide-topbar auth-topbar">
      <div className="titlebar-brand" aria-label="iiimage Studio">
        <span className="titlebar-brand-icon" aria-hidden="true">
          <span className="iiimage-logo-mark">
            <i />
            <i />
            <i />
          </span>
        </span>
        <strong>iiimage Studio</strong>
      </div>
    ...
    <main className="auth-shell" onPasteCapture={blockImagePaste}>
      ...
      <div className="auth-stage">
        <section className="auth-card boot-card" aria-label="启动检查">
          ...
          <span className="eyebrow">iiimage</span>
          <h1>正在启动</h1>
          <p>{message}</p>
    ...
    <main className="auth-shell" onPasteCapture={blockImagePaste}>
      ...
      <div className="auth-stage">
        <section className="auth-card" aria-label="登录注册">
          ...
          <span className="eyebrow">iiimage Studio</span>
          <h1>{authDraft.mode === "register" ? "创建 iiimage 账户" : "登录 iiimage Studio"}</h1>
