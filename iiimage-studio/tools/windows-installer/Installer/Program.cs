using Iiimage.WindowsInstaller;
    ...
    internal static int Run(string destination)
    {
        try
        {
            ...
            var metadataClean = !string.IsNullOrWhiteSpace(productVersion) &&
                                ...
                                string.Equals(versionInfo.ProductName, "iiimage Studio", StringComparison.Ordinal) &&
...
internal sealed class ExistingInstallation
{
    ...
    internal static ExistingInstallation? Find()
    {
        ...
        var hives = string.Equals(
            Environment.GetEnvironmentVariable("IIIMAGE_INSTALLER_DIAGNOSTIC_IGNORE_MACHINE"),
            "1",
        ...
        foreach (var hive in hives)
        {
            foreach (var view in new[] { RegistryView.Registry64, RegistryView.Registry32 })
            {
                try
                {
                    ...
                    if (uninstallKey is null || !File.Exists(Path.Combine(location, "iiimage Studio.exe"))) continue;
...
internal static class ShortcutState
{
    ...
    internal static IEnumerable<string> DesktopPaths()
    {
        ...
        if (!string.IsNullOrWhiteSpace(desktop)) yield return Path.Combine(desktop, "iiimage Studio.lnk");
        var profile = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile);
        if (!string.IsNullOrWhiteSpace(profile))
            yield return Path.Combine(profile, "OneDrive", "Desktop", "iiimage Studio.lnk");
    }
    ...
    internal static IEnumerable<string> StartMenuPaths()
    {
        ...
        yield return Path.Combine(programs, "iiimage Studio.lnk");
        yield return Path.Combine(programs, "iiimage Studio", "iiimage Studio.lnk");
    }
}
...
{
    private const string CoreResource = "iiimage.core-installer.exe";
    private const string HashResource = "iiimage.core-installer.sha256";
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!BrandOperationLock.TryAcquire(out operationLease, out var operationToken))
        {
            ...
            return new InstallResult(1618, false, false,
                "另一个 iiimage Studio 安装、更新或卸载操作正在进行，请等待其完成后重试。", logPath);
        }
        ...
        var tempDirectory = Path.Combine(Path.GetTempPath(), "iiimage-studio-installer", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempDirectory);
        var corePath = Path.Combine(tempDirectory, "iiimage-core-installer.exe");
        ...
        try
        {
                ...
                InstallerVersionPolicy.IsDowngrade(existing.Version, candidateVersion))
            {
                throw new InvalidOperationException(
                    $"这台电脑已安装较新的 iiimage Studio {existing.Version}。为保护项目兼容性，不能使用 {candidateVersion} 覆盖降级；请下载最新安装包。" );
            }
            ...
            var installedExe = Path.Combine(installDirectory, "iiimage Studio.exe");
            if (!File.Exists(installedExe)) throw new InvalidOperationException("安装完成后未找到 iiimage Studio.exe。");
            ApplyShortcutPreferences(desktopShortcut, startMenuShortcut);
            progress?.Report(new InstallProgress(100, "安装完成", "iiimage Studio 已准备就绪"));
            await WriteAllTextAsync(logPath, log.ToString());
        ...
        Action heartbeat)
    {
        for (var attempt = 0; attempt < 2; attempt++)
        {
            ...
            var diagnosticStall = string.Equals(
                Environment.GetEnvironmentVariable("IIIMAGE_INSTALLER_DIAGNOSTIC_STALL_CORE"),
                "1",
                StringComparison.Ordinal) &&
                string.Equals(
                    Environment.GetEnvironmentVariable("IIIMAGE_INSTALLER_DIAGNOSTIC_ALLOW_TIMEOUT_OVERRIDE"),
                    "1",
                    StringComparison.Ordinal);
    ...
    private static void ValidateInstallDirectory(string directory)
    {
        ...
        foreach (var managedRoot in managedDataRoots.Where(value => !string.IsNullOrWhiteSpace(value)))
        {
            if (IsWithin(directory, managedRoot) || IsWithin(managedRoot, directory))
                throw new InvalidOperationException("安装目录不能与 iiimage Studio 的项目、会话、设置或缓存目录重叠。请选择默认位置或其他专用文件夹。");
        }
    }
    ...
    {
        var brandedUninstaller = Path.Combine(installDirectory, "iiimage-studio-uninstaller.exe");
        var coreUninstaller = Path.Combine(installDirectory, "Uninstall iiimage Studio.exe");
        var uninstaller = File.Exists(brandedUninstaller) ? brandedUninstaller : coreUninstaller;
        if (File.Exists(uninstaller))
        {
            var temp = Path.Combine(Path.GetTempPath(), $"iiimage-rollback-{Guid.NewGuid():N}.exe");
            try
            {
...
internal sealed class InstallerWindow : BrandWindow
{
    ...
    internal InstallerWindow(string[] args) : base("iiimage Studio 安装程序", "iiimage Studio · " + DisplayVersion())
    {
        _args = args;
        ...
        _launchToggle = new BrandToggle("完成后启动 iiimage Studio", true);
    ...
    private void ShowCapturePage(string[] args)
    {
        ...
        switch (page)
        {
            ...
            case "error": ShowError("安装组件完整性校验失败，请重新从官网下载。", @"C:\Users\Demo\AppData\Local\iiimage Studio\installer-logs\setup.log"); break;
    ...
    private void ShowWelcome()
    {
        ...
        SetPage("WELCOME", _existing is null ? "欢迎使用 iiimage Studio" : "更新或修复 iiimage Studio",
            _existing is null
    ...
    private void ShowProgress()
    {
        ...
        SetPage("INSTALLING", _existing is null ? "正在安装 iiimage Studio" : "正在安全更新 iiimage Studio",
    ...
    private void ShowComplete(bool wasUpgrade)
    {
        ...
        SetPage("READY", "iiimage Studio 已准备就绪", "所有程序组件与系统入口均已配置完成。", stack);
        BackButton.Visibility = Visibility.Collapsed;
    ...
    private async Task BeginInstallAsync()
    {
        ...
        if (_cancelFreshInstall && result.Success)
        {
            ...
            if (!rollbackCompleted)
            {
                ...
                CloseEnabled = true;
                ShowError(
                    "安装已经完成，但自动撤销未能完整移除程序。请关闭 iiimage Studio，并从 Windows“已安装的应用”再次卸载。",
                    Path.Combine(
                        Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
                        "iiimage Studio",
                        "installer-logs"));
    ...
    {
        var installedExecutable = Path.Combine(installDirectory, "iiimage Studio.exe");
        var timeout = ProcessWatchdog.ResolveTimeout(
            "IIIMAGE_INSTALLER_DIAGNOSTIC_CANCEL_ROLLBACK_TIMEOUT_MS",
            TimeSpan.FromMinutes(5));
        for (var attempt = 0; attempt < 2; attempt++)
        {
            var wrapper = Path.Combine(installDirectory, "iiimage-studio-uninstaller.exe");
            var core = Path.Combine(installDirectory, "Uninstall iiimage Studio.exe");
    ...
    private void BrowseFolder()
    {
        ...
        if (ModernFolderPicker.TryPick(this, "选择 iiimage Studio 安装位置", _pathBox.Text, out var selectedPath))
        {
            _pathBox.Text = string.Equals(Path.GetFileName(selectedPath), "iiimage Studio", StringComparison.OrdinalIgnoreCase)
                ? selectedPath
                : Path.Combine(selectedPath, "iiimage Studio");
        }
    }
    ...
    private static Border CreateReleaseNotesCard(double scrollHeight)
    {
        ...
        root.Children.Add(MakeScrollArea(noteStack, scrollHeight, $"iiimage Studio {version} 发布说明"));
