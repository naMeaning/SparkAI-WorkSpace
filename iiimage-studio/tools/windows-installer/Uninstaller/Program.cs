...
internal sealed class UninstallArguments
{
    ...
    internal static UninstallArguments Parse(string[] args)
    {
        ...
        var diagnosticOverrideAllowed = string.Equals(
            Environment.GetEnvironmentVariable("IIIMAGE_INSTALLER_DIAGNOSTIC_ALLOW_DATA_OVERRIDE"),
            "1",
            StringComparison.Ordinal);
        ...
        foreach (var hive in new[] { RegistryHive.CurrentUser, RegistryHive.LocalMachine })
        {
            foreach (var view in new[] { RegistryView.Registry64, RegistryView.Registry32 })
            {
                try
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(value) && File.Exists(Path.Combine(value, "iiimage Studio.exe"))) return value;
                }
                catch { }
            }
        }
...
internal static class UninstallerBootstrap
{
    internal static bool RelaunchOutsideInstallIfNeeded(string[] args, UninstallArguments options)
    {
        ...
        try
        {
            ...
            tempRoot = Path.Combine(Path.GetTempPath(), "iiimage-studio-uninstaller", "ui-" + Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(tempRoot);
            var detachedPath = Path.Combine(tempRoot, "iiimage-studio-uninstaller.exe");
...
internal static class UninstallerEngine
{
    ...
    internal static async Task<UninstallResult> UninstallAsync(UninstallArguments options, IProgress<UninstallProgress>? progress)
    {
        ...
        var tempRoot = Path.Combine(Path.GetTempPath(), "iiimage-studio-uninstaller", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempRoot);
        var tempCore = Path.Combine(tempRoot, "iiimage-core-uninstaller.exe");
        try
        {
            ...
            var core = Path.Combine(options.InstallDirectory, "Uninstall iiimage Studio.exe");
            if (!File.Exists(core))
            {
                if (!File.Exists(Path.Combine(options.InstallDirectory, "iiimage Studio.exe")))
                {
            ...
            var coreTimeout = ProcessWatchdog.ResolveTimeout(
                "IIIMAGE_UNINSTALLER_DIAGNOSTIC_CORE_TIMEOUT_MS",
                DefaultCoreTimeout);
            var recoveryTimeout = ProcessWatchdog.ResolveTimeout(
                "IIIMAGE_UNINSTALLER_DIAGNOSTIC_RECOVERY_TIMEOUT_MS",
            ...
            var installedExecutable = Path.Combine(options.InstallDirectory, "iiimage Studio.exe");
            ...
            if (File.Exists(installedExecutable))
                throw new InvalidOperationException("程序仍在运行，未能完整移除。请关闭 iiimage Studio 后重试。");
        ...
        Action heartbeat)
    {
        ...
        throw new TimeoutException("卸载内核长时间未响应，已停止本次操作。请关闭 iiimage Studio 后重试；重复执行会继续清理未完成的组件。");
    }
    ...
    private static void ScheduleCleanupScript(string filePath, string directoryPath)
    {
        try
        {
            ...
            var scriptRoot = Path.Combine(Path.GetTempPath(), "iiimage-studio-uninstaller", "cleanup");
            Directory.CreateDirectory(scriptRoot);
            var scriptPath = Path.Combine(scriptRoot, "cleanup-" + Guid.NewGuid().ToString("N") + ".cmd");
            File.WriteAllText(scriptPath,
                "@echo off\r\n" +
                "ping 127.0.0.1 -n 3 > nul\r\n" +
                "del /f /q \"%IIIMAGE_DELETE_FILE%\" > nul 2>&1\r\n" +
                "rmdir \"%IIIMAGE_DELETE_DIR%\" > nul 2>&1\r\n" +
                "del /f /q \"%~f0\" > nul 2>&1\r\n",
                Encoding.ASCII);
            ...
            start.EnvironmentVariables["IIIMAGE_DELETE_FILE"] = normalizedFile;
            start.EnvironmentVariables["IIIMAGE_DELETE_DIR"] = normalizedDirectory;
            Process.Start(start);
...
internal sealed class UninstallerWindow : BrandWindow
{
        ...
        : base("iiimage Studio 卸载程序", "安全卸载")
    {
        ...
        switch (capturePage)
        {
            ...
            case "error": ShowError("iiimage Studio 仍在运行，请关闭后重试。", @"C:\Users\Demo\AppData\Local\iiimage Studio\installer-logs\uninstall.log"); break;
    ...
    private void ShowConfirm()
    {
        ...
        SetPage("UNINSTALL", "卸载 iiimage Studio", "移除程序组件与系统快捷入口。你可以明确决定是否保留本地创作数据。", stack);
        BackButton.Visibility = Visibility.Collapsed;
    ...
    private void ShowProgress()
    {
        ...
        SetPage("REMOVING", "正在卸载 iiimage Studio", "请保持此窗口开启，卸载器会先关闭应用再安全移除组件。", stack);
        BackButton.Visibility = Visibility.Collapsed;
    ...
    private void ShowComplete(bool preserve)
    {
        ...
        var title = Text("iiimage Studio 已从这台电脑移除。", 17, BrandPalette.Text, FontWeights.SemiBold);
        ...
        FooterNote.Text = preserve ? "感谢使用 iiimage Studio" : "本地数据清理已完成";
    }
