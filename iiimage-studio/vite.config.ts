...
const studioIconPlugin: Plugin = {
  name: "iiimage-studio-build-icon",
  apply: "build" as const,
  buildStart() {
    this.emitFile({
      type: "asset",
      fileName: "iiimage-studio.png",
      source: readFileSync(new URL("./public/iiimage-studio.png", import.meta.url))
    });
  }
};
...
export default defineConfig(({ command, mode }) => ({
  ...
  define: {
    __IIIMAGE_AIDEBUG__: JSON.stringify(command === "serve"),
    __IIIMAGE_PERF_PROBE__: JSON.stringify(command === "build" && mode === "performance")
  },
  build: {
    copyPublicDir: false,
    rolldownOptions: {
  ...
  server: {
    ...
    proxy: {
      "/__iiimage_new_api": {
        ...
        cookieDomainRewrite: "",
        cookiePathRewrite: "/",
        rewrite: (path: string) => path.replace(/^\/__iiimage_new_api/, "")
      },
      "/ai-api": {
        target: "https://fast.aieyra.cn",
        changeOrigin: true,
        secure: true,
