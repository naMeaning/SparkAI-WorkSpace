...
const desktopDownloadPath = "/downloads/naimage-studio/windows"

func registerDesktopDownloadRoutes(router *gin.Engine) {
	router.GET(desktopDownloadPath, middleware.RouteTag("desktop-download"), controller.DownloadDesktopInstaller)
}
...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		desktopDownloadRoute := apiRouter.Group("/desktop-download")
		desktopDownloadRoute.Use(middleware.UserAuth())
		{
			desktopDownloadRoute.POST("/captcha", middleware.CriticalRateLimit(), controller.CreateDesktopDownloadCaptcha)
			desktopDownloadRoute.POST("/authorize", anonymousRequestBodyLimit, controller.AuthorizeDesktopDownload)
		}
		desktopUpdateRoute := apiRouter.Group("/desktop-update")
		desktopUpdateRoute.Use(middleware.UserAuth())
		{
			desktopUpdateRoute.GET("/check", controller.CheckDesktopUpdate)
			desktopUpdateRoute.POST("/authorize", middleware.CriticalRateLimit(), anonymousRequestBodyLimit, controller.AuthorizeDesktopRestartUpdate)
		}
		desktopClientRoute := apiRouter.Group("/desktop-client")
		desktopClientRoute.Use(middleware.UserAuth())
		{
			desktopClientRoute.POST("/events", anonymousRequestBodyLimit, controller.RecordDesktopClientEvent)
		}

		// Embedded CRM distribution system. Auth is always owned by new-api;
		// the proxy injects signed internal headers for the CRM API.
		apiRouter.Any("/crm/*path", middleware.UserAuth(), controller.CrmProxy)
