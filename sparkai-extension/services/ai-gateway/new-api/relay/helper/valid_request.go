...
func invalidImageCountError() error {
	return fmt.Errorf("n is invalid: n must be an integer between 1 and %d", dto.MaxImageN)
}
...
func GetAndValidateTextRequest(c *gin.Context, relayMode int) (*dto.GeneralOpenAIRequest, error) {
	...
	if exceedsMaxTokensLimit(textRequest.MaxCompletionTokens) {
		return nil, errors.New("max_tokens is invalid: max_completion_tokens is invalid")
	}
	if textRequest.Model == "" {
		return nil, errors.New("model is required")
	}
