...
type interruptedImageResponseReader struct {
	written bool
}

func (reader *interruptedImageResponseReader) Read(value []byte) (int, error) {
	if reader.written {
		return 0, errors.New("upstream image response interrupted")
	}
	reader.written = true
	return copy(value, []byte(`{"data":[`)), nil
}
...
func TestOpenaiImageStreamHandlerClientDisconnectKeepsRequestedCount(t *testing.T) {
	...
	require.True(t, c.GetBool(common.ManagedImageOutcomeUnknownKey))
...
func TestOpenaiImageHandlersReturnJSONError(t *testing.T) {
	...
	t.Run("non-streaming handler", func(t *testing.T) {
		...
		require.False(t, c.GetBool(common.ManagedImageOutcomeUnknownKey))
...
func TestOpenaiImageHandlersRejectEmptySuccessfulImagePayloads(t *testing.T) {
	oldMode := gin.Mode()
	gin.SetMode(gin.TestMode)
	t.Cleanup(func() { gin.SetMode(oldMode) })
	...
	for _, testCase := range []struct {
		...
		{name: "non-streaming empty data", body: `{"data":[]}`, handler: OpenaiImageHandler},
		{name: "non-streaming item without asset", body: `{"data":[{"revised_prompt":"nothing generated"}]}`, handler: OpenaiImageHandler},
		{name: "JSON as stream empty data", body: `{"data":[]}`, stream: true, handler: OpenaiImageJSONAsStreamHandler},
		{name: "JSON as stream item without asset", body: `{"data":[{"revised_prompt":"nothing generated"}]}`, stream: true, handler: OpenaiImageJSONAsStreamHandler},
	} {
		t.Run(testCase.name, func(t *testing.T) {
			c, recorder, resp, info := newImageTestContext(t, testCase.body, "application/json", testCase.stream)
			usage, apiErr := testCase.handler(c, info, resp)
			require.Nil(t, usage)
			require.NotNil(t, apiErr)
			require.Equal(t, http.StatusBadGateway, apiErr.StatusCode)
			require.Equal(t, types.ErrorCodeBadResponseBody, apiErr.GetErrorCode())
			require.ErrorContains(t, apiErr, "does not contain a usable image asset")
			require.True(t, c.GetBool(common.ManagedImageOutcomeUnknownKey))
...
func TestOpenaiImageStreamHandlerRejectsDoneWithoutImageAsset(t *testing.T) {
	oldMode := gin.Mode()
	gin.SetMode(gin.TestMode)
	t.Cleanup(func() { gin.SetMode(oldMode) })
	oldTimeout := constant.StreamingTimeout
...
func TestOpenaiImageHandlersRejectOversizedUpstreamBodies(t *testing.T) {
	oldMode := gin.Mode()
	gin.SetMode(gin.TestMode)
	t.Cleanup(func() { gin.SetMode(oldMode) })
	t.Setenv("MANAGED_IMAGE_IDEMPOTENCY_MAX_RESPONSE_MB", "1")
	...
	for _, testCase := range []struct {
		...
		{name: "non-streaming handler", handler: OpenaiImageHandler},
		{name: "JSON as stream handler", stream: true, handler: OpenaiImageJSONAsStreamHandler},
	} {
		t.Run(testCase.name, func(t *testing.T) {
			c, recorder, resp, info := newImageTestContext(t, body, "application/json", testCase.stream)
			usage, apiErr := testCase.handler(c, info, resp)
			require.Nil(t, usage)
			require.NotNil(t, apiErr)
			require.Equal(t, http.StatusBadGateway, apiErr.StatusCode)
			require.Equal(t, types.ErrorCodeBadResponseBody, apiErr.GetErrorCode())
			require.Contains(t, apiErr.Error(), "exceeds the configured size limit")
			require.True(t, c.GetBool(common.ManagedImageOutcomeUnknownKey))
			require.True(t, types.IsSkipRetryError(apiErr))
			require.Empty(t, recorder.Body.String())
		})
	...
	require.Nil(t, usage)
	require.NotNil(t, apiErr)
	require.False(t, c.GetBool(common.ManagedImageOutcomeUnknownKey), "an explicit non-2xx upstream failure may follow the normal retry policy")
	require.False(t, types.IsSkipRetryError(apiErr))
	require.Empty(t, recorder.Body.String())
}

func TestOpenaiImageHandlersMarkMalformedOrInterrupted2xxResponseAmbiguous(t *testing.T) {
	oldMode := gin.Mode()
	gin.SetMode(gin.TestMode)
	t.Cleanup(func() { gin.SetMode(oldMode) })
	...
	for _, testCase := range []struct {
		...
		{name: "malformed non-streaming JSON", body: io.NopCloser(strings.NewReader(`{"data":[`)), handler: OpenaiImageHandler},
		{name: "malformed JSON as stream", stream: true, body: io.NopCloser(strings.NewReader(`{"data":[`)), handler: OpenaiImageJSONAsStreamHandler},
		{name: "interrupted non-streaming body", body: io.NopCloser(&interruptedImageResponseReader{}), handler: OpenaiImageHandler},
		{name: "interrupted JSON as stream body", stream: true, body: io.NopCloser(&interruptedImageResponseReader{}), handler: OpenaiImageJSONAsStreamHandler},
	} {
		t.Run(testCase.name, func(t *testing.T) {
			c, recorder, resp, info := newImageTestContext(t, "", "application/json", testCase.stream)
			resp.Body = testCase.body
			usage, apiErr := testCase.handler(c, info, resp)
			require.Nil(t, usage)
			require.NotNil(t, apiErr)
			require.True(t, c.GetBool(common.ManagedImageOutcomeUnknownKey))
