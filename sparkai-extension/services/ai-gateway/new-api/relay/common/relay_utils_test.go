...
func TestValidateBasicTaskRequestRejectsHugeDuration(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := `{"model":"veo-3.0-generate-001","prompt":"make a video","duration":3601}`

	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/videos", strings.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")

	err := ValidateBasicTaskRequest(c, &RelayInfo{}, "text_generate")
	require.NotNil(t, err)
	require.Equal(t, "invalid_seconds", err.Code)
}
