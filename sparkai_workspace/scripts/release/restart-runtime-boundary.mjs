...
export function readPackagedUpdateMetadata(appRoot) {
  ...
  const compatibility = String(metadata.naimageUpdateCompatibility || "").trim();
...
export function verifyRestartRuntimeBoundary({ baselineRoot, candidateRoot, expectedCompatibility = "" }) {
  ...
  if (runtimeChanged && !compatibilityChanged) {
    ...
    throw new Error(
      `Restart-unsafe runtime files changed without an naimageUpdateCompatibility bump: ${changedPaths.slice(0, 12).join(", ") || "unknown"}`
    );
  }
  return {
    ok: true,
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    const report = verifyRestartRuntimeBoundary({
      baselineRoot,
      candidateRoot,
      expectedCompatibility: String(packageMetadata.naimageUpdateCompatibility || "").trim()
    });
