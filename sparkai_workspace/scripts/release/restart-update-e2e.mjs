...
const expectedAsar = resolve(valueArg(
  "--expected-asar",
  join(projectRoot, "release", `naimage-Restart-Update-${releaseVersion}-x64.asar`)
));
...
try {
  ...
  const setup = await evaluate(client, `(async () => {
    ...
    if (!(await waitUntil(() => Boolean(window.naimageConfig && window.naimageAgent && window.naimageUpdater)))) {
    ...
    const memory = await window.naimageAgent.saveFastMemory({ projectId, conversationId, text: marker });
    const settingsResponse = await window.naimageConfig.loadSettings();
    const settingsSaved = await window.naimageConfig.saveSettings({ ...(settingsResponse?.settings || {}), timeoutSeconds: 181 });
  ...
  if (seedPendingSource) {
    const seededStatus = await evaluate(client, "window.naimageUpdater.status()", 60_000);
    effectiveChecked = {
      ok: true,
    ...
    effectiveChecked = await evaluate(client, "window.naimageUpdater.check()", 60_000);
  }
  ...
  const downloaded = seedPendingSource
    ? await evaluate(client, "window.naimageUpdater.status()", 60_000)
    : await evaluate(client, "window.naimageUpdater.downloadRestart()", 360_000);
  ...
  stage = "apply-update";
  const applied = await evaluate(client, "window.naimageUpdater.applyRestart()", 60_000);
  ...
  const backupRemoved = !existsSync(`${targetAsar}.naimage-backup`);
