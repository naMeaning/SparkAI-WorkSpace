...
function stagingFiles(outputDir) {
  ...
  return readdirSync(outputDir).filter((entry) => entry.startsWith(".naimage-import-") && entry.endsWith(".tmp"));
}

async function main() {
  const root = mkdtempSync(path.join(os.tmpdir(), "naimage-image-import-"));
  ...
  try {
    for (let index = 0; index < 500; index += 1) {
      const suffix = Buffer.from(`naimage-fixture-${String(index).padStart(4, "0")}`);
    ...
    writeFileSync(stallingWorkerPath, `
      ...
      process.once("message", (payload) => {
        ...
        writeFileSync(path.join(payload.outputDir, ".naimage-import-" + payload.requestId + "-stall.tmp"), "staging");
