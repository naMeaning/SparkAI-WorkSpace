...
当前 Goal（2026-09-27）：Agent Composer 的明确发送确认、设置表面可读性和图片协议自动适配。图片模型的协议、网关、传输和能力字段继续作为兼容输入，但用户可见配置应逐步收敛到模型、Base URL 和凭据；解析与请求构造仍由 Main/runtime 持有，Renderer 不接触秘密。
...
image_gen
  ...
  → 官方 OpenAI 兼容 Images API
  ├─ 生成：POST /v1/images/generations（JSON）
  ├─ 编辑/参考图/蒙版：POST /v1/images/edits（multipart，GPT Image 使用 image[]）
  └─ 账号登录与自定义 Base URL（New API / sub2api / 球球 Token）走同一 Images 合同
  → 项目 output 资产
  → workflow action
  → Renderer 归组与溯源
```
...
- `account`：用户名/密码登录 SparkAPI/New API 后即可进入工作区，不请求设备 License。原生 New API `v1.0.0-rc.23` 返回 Bearer access token、HttpOnly `new_api_refresh` cookie 与 auth session ID；Electron Main 在到期前或受保护请求明确返回 401 时合并并发刷新，并只重放一次。旧部署继续兼容 `session` cookie + `New-Api-User`。`completeNewApiLogin()` 的关键路径只等待 `/api/user/login`、认证 bundle 安全持久化和 `cacheOnly` 模型设置，用户资料、账户 Token/额度与模型目录由 Renderer 打开工作区后的 `refreshServerState()` 后台预热；不得重新把这些后置请求串行放回登录响应。用户认证凭据只用于 `/api/user/*`、`/api/token/*` 和更新；模型 `/v1/*` 始终使用模型连接解析出的 Key。桌面列出、创建、编辑、分组、启停和删除用户密钥，并用用户选择的完整 Key 直连 `accountBaseUrl/v1`。完整 Key 优先兼容原生 New API 在 token 列表/详情中的返回值，脱敏部署则通过 `POST /api/token/:id/key` 按需取回；两者都只缓存在 Electron Main 内存，不进入 Renderer、设置文件、模型缓存或日志。`account-token-cache.json` 只按账户地址 + user ID 保存最多 8 份公开元数据快照，不保存任何 Key、Cookie、IP 白名单或模型限制。
...
设置页数据加载是显式的离线优先边界：挂载时 `tokens({ preferCached: true })` 与 `models({ cacheOnly: true })` 只读 Main 本地快照；没有模型快照时回退到设置内模型池。切换设置分区不会联网；只有“刷新密钥与分组”“刷新模型”、密钥 CRUD 或切换密钥等明确用户动作可以请求 New API。模型目录运行缓存 TTL 为 15 分钟；过期后先继续展示旧目录，后台刷新。`cacheOnly` 磁盘快照即使超过 TTL 也可离线显示，并标记来源/更新时间。

对话模型和生图模型共用 `ModelConnectionBinding` 结构，但分别持久化为 `agentModelBindings` 与 `imageModelBindings`。两种模式都允许每个已选模型独立覆盖 Base URL/API Key，字段留空时分别继承全局连接或账号凭据；账号模式的 Key 回退顺序为“模型自定义 Key → 模型绑定账户 Token → 全局账户 Token”。SparkAPI 专用版由 Main 访问策略固定官方账号地址并拒绝逐模型 Base URL，但保留账号登录后的逐模型 API Key。普通 Chat Completions 和 `/v1/responses` 对话都按请求体中的实际 `model` 解析对话绑定；Responses 生图继续由图片 provider 取图片连接，不能让顶层对话模型绑定抢占图片凭据。provider Authorization 由解析后的连接唯一生成，调用方传入的同名或大小写变体 header 会被移除，rc.23 的账户 access token 也不得覆盖模型级自定义 Key。Main 在模型列表草稿合并前用当前已解密设置恢复 Renderer 占位符；逐模型 Key、全局 Key 和 rc.23 auth bundle 一样只进入 Windows `safeStorage` sidecar，普通 JSON、Renderer、日志和模型缓存均不含明文。模型缓存身份包含逐模型 Base URL/Key/Token 的 SHA-256 指纹，更换任一绑定后不得复用旧目录。无 Main 注入的 Agent Runtime 直连回退只在 `accessMode=custom` 时应用逐模型 URL/Key；账号 Token 始终由 Main 解析。最低验证为 `test:agent-model-binding`、`test:custom-api-transport`、`test:settings-secret-store`、`test:model-catalog`、`test:settings-lazy-load`、`test:access-variant`、`test:license` 和 `typecheck`。

账号模式与自定义 Base URL 的生图默认都直接调用官方兼容 Images API。`sparkai-extension` 仍提供同域 `/v1/image-tasks*` 作为可选长任务包装：Bearer Key 只留单进程内存，SQLite 仅保存 HMAC owner 与任务状态，后台经 Docker 内网调用原生 `/v1/images/generations`。桌面客户端不再把这条任务化路径当作默认协议。编辑、参考图和蒙版始终走 `/v1/images/edits`。
...
| `src/project-agent-composer.tsx` | 自然异步的主 Agent 输入表面与运行控制；模型、素材、当前模式、比例值和清晰度值组成紧凑玻璃工具栏，原图/参考图动作进入素材菜单，普通/Goal 共用单触发器和双瓣扇形菜单，画幅选项使用自定义 listbox；composer 以自身 container query 在窄面板中切换两列/整行布局，菜单保持视口内，普通用户运行中只选择自动处理、只修改要求或更换处理图片，内部仍映射兼容 TaskScope 协议，每次 steer/运行结束重置自动模式 | Agent 执行、画布 mutation、密钥读取、直接网络请求、IPC 或 run record | `ProjectAgentComposer` | `test:settings-lazy-load`, `test:model-catalog`, `test:agent-panel-ui`, `test:agent-text-ui`, `test:workspace-glass-ui`, `typecheck`, `build`, `test:bundle` |
...
| `src/ui/glass-select.tsx` | 设置与模型配置共用的受控 Glass listbox；通过 Portal 渲染菜单，保留选中/禁用状态，支持 Arrow/Home/End、Enter/Space、Escape/Tab 和关闭后焦点返回，避免原生 Windows select 弹出白底 | 业务设置持久化、凭据读取、模型目录请求或第二份选择状态 | `GlassSelect`, `GlassSelectOption` | `test:workspace-glass-ui`, `test:ui-foundation`, `typecheck`, `build` |
...
| `src/glass-lab.tsx` | 当前设置外观页：七套彩色玻璃主题、材质、背景与预览 | 全局 App 状态、Glass token/surface、独立 Agent appearance 快照 | `GlassLab`, `data-glass-theme`, `data-glass-material` | `test:glass-theme`, `test:settings-lazy-load`, `test:workspace-glass-ui`, `aidebug:glass-workspace` |
...
| `src/styles.css`, `src/styles/01…08` | 有序样式入口与 base/canvas/legacy/dialog/desktop/responsive/workbench/motion 区域；`01-base-controls.css` 持有共享字体 token 与 GlassSelect 基础 listbox，`01-liquid-glass-tokens.css` 持有 Glass token/旧 `--theme-*` 桥，07a→07i 仍由 workbench 聚合，`07j-liquid-glass-surfaces.css` 在其后持有 Glass surface、近实色搜索菜单、GlassSelect 触发器/菜单和 reduced-motion 直接操作例外；`02-canvas-workspace.css` 的普通拖动只消费 `--node-drag-x/y`，`image-layout-settle` 不得声明 transform，密集 viewport 交互只在 `is-viewport-detail-reduced` 生命周期内停绘媒体、关系线和高成本阴影；共享 `.ui-surface` 消费组件局部可覆盖的 `--ui-surface-radius`，设置/Agent 窄表面使用 container query 重排 | 数据修复、运行时状态补丁、静止态画布 artwork 滤镜、拖动 transition 延迟、布局动画占用 transform、交互结束后残留降载 class、跨文件随意改 import 顺序、固定重置所有功能表面圆角，或把 `04b-glass-lab.css` 提前放入全局首屏入口 | `@import`, `--font-*`, `--ui-surface-radius`, `--node-drag-x`, `--node-drag-y`, `is-viewport-detail-reduced`, `01-liquid-glass-tokens.css`, `07-workbench-flattening.css`, `07j-liquid-glass-surfaces.css`, `08-motion-accessibility.css`, `@container project-agent-composer`, `@container settings-drawer` | `test:workspace-glass-ui`, `test:ui-foundation`, `test:settings-lazy-load`, `test:agent-panel-ui`, `test:aidebug-glass-workspace`, `aidebug:glass-workspace`, `aidebug:performance`, `test:bundle` |
...
| `src/main.tsx`（画布拖动编排） | 单节点、分层组、全图片容器多选和多选普通单图归组的指针状态；普通单图拖入容器时从 canonical selection 收集完整资产集合，回滚 transform 后以一次 layout mutation 原子提交，保留来源关系；批量容器拖动用一组起点快照驱动 transform 预览、整组关系线重算、单次 `setNodes` 提交与 `pointercancel` 回滚 | `dragRef.multiNodeStart`, `draggingNodeIdsRef`, `selectedAssetMembers`, `moveContainerAssets`, `applyTransientNodePositions`, `beginNodeDrag`, `moveNode`, `endNodeDrag` | `test:selection`, `test:image-container`, `test:image-layout`, `aidebug:image-collection`, `test:workspace-glass-ui`, `typecheck`, `build` |
...
| `src/canvas-relation-graph.ts` | 画布关系连接/断开纯逻辑、关系类型与 Requirement input role 校验、stale edge 防护和可见端点映射 | React pointer lifecycle、菜单/IPC、项目持久化 | `connectCanvasRelations`, `disconnectCanvasRelations`, `relationForCanvasEdge` | `test:requirement-graph`, `test:workspace-glass-ui`, `typecheck` |
...
画布鼠标批量移动只在当前选择映射到两个或更多可见宿主、且全部宿主均为 `imageContainer` 或 `imageCollection` 时成立。按下节点必须属于该选择；所有宿主共享同一个世界坐标 `dx/dy`，预览阶段只写 `--node-drag-x/y` 并以整组新端点一次重算受影响 provenance path，React 节点坐标保持不变。松手后一次映射提交全部宿主坐标并保留选择，`pointercancel` 和全局交互取消恢复全部 transform、关系线、z-index 与 `will-change`。普通单图的多选归组是另一条受限路径：仅当完整 canonical selection 都是可归组的普通单图时启用，目标命中后先清除临时位移，再把所有选中资产与目标容器通过一次 layout mutation 合并；源节点全部排除为目标，`parentId`/`relationType` 必须保持不变，失败不得部分写入。未选中容器、混合选择、普通单节点、图片瓦片提取和分层 PNG 继续使用各自既有路径。
...
| `src/settings-drawer.tsx` | 接入/外观/模型/Agent/更新设置表面；账户密钥选择使用 `GlassSelect`，窄抽屉的分页标签按自身宽度重排 | `test:workspace-glass-ui`, `test:settings-lazy-load`, `test:settings-persistence`, `typecheck` |
| `src/model-config-dialog.tsx` | Agent/图片模型选择、服务端模型展示和逐模型接入绑定（Base URL/API Key 或账户密钥）；账户密钥使用与设置页相同的 `GlassSelect` | `test:agent-text-ui`, `test:model-catalog`, `test:settings-lazy-load`, `test:workspace-glass-ui`, `aidebug:gui` |
...
| 图片容器/布局与最终图查看 | `image-container-spec.ts`, `image-container-graph.ts`, `task-result-layout.ts`, `image-layout.ts`, `src/image-viewer.tsx`, `src/selection-reference-images.ts`, `src/project-agent-composer.tsx`, `src/styles/04-dialogs-viewers.css`；`image-container.ts` 仅作兼容 façade | session migration、TaskScope provenance、layout projection；画布选中图片/容器进入 Agent 素材条，可改序号和原图/参考角色；容器标题栏与右键可按当前提示词重新生图且不覆盖原节点；`image-layout-settle` 不得占用 drag transform；查看器过滤 pending/error/partial，只显示最终资产，上方大图与下方单行横向缩略条保持稳定 | `test:image-container`, `test:image-layout`, `test:workspace-glass-ui`, `typecheck`, `aidebug:glass-workspace` |
...
| 模型目录/缓存与逐模型连接 | `desktop/{model-catalog,new-api-client,settings-secret-store}.cjs`, `electron-main.cjs`, `agent-runtime.cjs`, `src/{core,settings-persistence,server,main,project-agent-composer,settings-drawer,model-config-dialog}.ts*`, `docs/NEWAPI_INTEGRATION.md` | 服务响应 DTO、当前账户 Token 或自定义接口的 `/v1/models`、每模型 `supported_endpoint_types`、独立 Agent/Image/Video 状态、能力证据与最近验证时间；`agentModelBindings`/`imageModelBindings` 分域路由，留空继承全局 URL/Key/Token，Responses 生图不得读取对话绑定；15 分钟运行缓存、过期先用旧目录后台刷新、`cacheOnly` 离线磁盘快照和大列表虚拟化；缓存键含逐模型 URL/Key/Token 的单向指纹，更换绑定不得复用旧目录，磁盘缓存仍不得含 Key；Renderer 占位符合并前由 Main 恢复；端点能力是正向证据，缺失或错误时保留图像/视频名称回退，显式视频模型不得混入前两类 | `test:agent-model-binding`, `test:custom-api-transport`, `test:model-catalog`, `test:settings-persistence`, `test:settings-secret-store`, `test:settings-lazy-load`, `test:access-variant`, `test:ipc-registration`, `typecheck` |
| Unified Image Generation Layer | `runtime/image-generation/{types,adapters,normalize-response,errors}.cjs`, `desktop/image-generation-service.cjs`, `desktop/new-api-client.cjs`, `electron-main.cjs`, `agent-runtime.cjs`, `src/{core,settings-persistence,model-config-dialog,project-agent-composer,manual-image-task-dialog}.tsx` | UI/Canvas/Agent 只提交统一 ImageGenerationRequest；已知模型由内部 preset 自动选择 OpenAI Images、xAI Images 或 Gemini Native，用户界面不再要求填写 protocol/gateway/transport/capability；旧 JSON 细节字段仍作为未知/自定义模型的兼容输入；Transport 继续负责 NewAPI/Sub2API/Direct、凭据、URL、超时和 async task；账号模式逐模型 Base URL/API Key 由 Main 解析，SparkAPI 专用版仍由 access policy 禁止自定义 Base URL；不把 API Key 或厂商响应暴露到 Renderer，异步创建不明确不得自动重建 | `test:image-generation-adapters`, `test:image-generation-service`, `test:image-generation-async`, `test:custom-api-transport`, `test:agent-model-binding`, `test:settings-secret-store`, `typecheck`, `build` |
...
| 外观 / Glass Lab | `glass-lab.tsx`, `glass-theme.ts`, `settings-persistence.ts`, `styles/{01-liquid-glass-tokens,04b-glass-lab,07j-liquid-glass-surfaces}.css` | `AppSettings.glassTheme/glassMaterial/glassParameters`、Electron 窗口底色、root dataset、设置懒加载、独立 Agent appearance 快照；旧 `theme-palette-picker.tsx` 不再是外观入口 | `test:glass-theme`, `test:settings-lazy-load`, `test:workspace-glass-ui`, `test:ipc-registration`, `aidebug:glass-workspace` |
...
| 远端 API/模型/登录 | `runtime/access-variant.cjs`, `src/access-policy.ts`, `desktop/ipc/server-ipc.cjs`, `desktop/new-api-transport.cjs`, `desktop/new-api-client.cjs`, `desktop/settings-secret-store.cjs`, `electron-main.cjs`, `src/server.ts` | 原生 New API rc.23 auth bundle 与旧 session 双协议、登录关键路径与后台预热分离、构建策略清单、preload/core bridge、sparkai-extension；SparkAPI 专用版不得接受自定义 Base URL/API Key 或 Relay 覆盖 | `test:new-api-login`, `test:custom-api-transport`, `test:account-token`, `test:settings-secret-store`, `test:settings-lazy-load`, `test:access-variant`, `test:ipc-registration`, `test:new-api-transport`, `test:lifecycle`, `typecheck`, `aidebug:auth-gate` |
...
| 路径/文件 | 内容 | 所有者 |
| --- | --- | --- |
| `app-settings.json` + `app-settings.secrets.json` | 普通文件保存 App 设置、Glass、账号协议/用户 ID/过期时间、所选账户密钥的 ID/名称/分组和代理等；safeStorage sidecar 加密保存自定义 Agent/图片/逐模型 API Key 及 rc.23 access token/refresh cookie/auth session ID | Electron Main；普通 JSON、Renderer、日志、模型缓存和项目文件不得含完整 Key 或认证 bundle 明文，Renderer 只接收自定义 Key 占位符 |
...
| 原生 New API rc.23 登录关键路径、旧 session 回退、刷新 single-flight/单次 401 重放/登出、凭据加密与模型级 Key 隔离 | `corepack pnpm run test:new-api-login`, `corepack pnpm run test:custom-api-transport`, `corepack pnpm run test:account-token`, `corepack pnpm run test:settings-secret-store`, `corepack pnpm run test:settings-persistence`, `corepack pnpm run test:settings-lazy-load`, `corepack pnpm run test:agent-model-binding`, `corepack pnpm run test:access-variant`, `corepack pnpm run test:ipc-registration`, `corepack pnpm run typecheck` |
