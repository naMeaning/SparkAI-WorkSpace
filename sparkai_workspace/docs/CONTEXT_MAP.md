...
| Unified Image Generation Layer | `runtime/image-generation/{types,adapters,normalize-response,errors}.cjs`, `desktop/image-generation-service.cjs`, `desktop/new-api-client.cjs`, `electron-main.cjs`, `agent-runtime.cjs`, `src/{core,settings-persistence,model-config-dialog,project-agent-composer,manual-image-task-dialog}.tsx` | UI/Canvas/Agent 只提交统一 ImageGenerationRequest；模型配置显式声明 provider/protocol/gateway/capabilities，Adapter 负责 OpenAI Images、xAI Images、Gemini Native 请求与响应，Transport 负责 NewAPI/Sub2API/Direct、凭据、URL、超时和 async task；账号模式逐模型 Base URL/API Key 由 Main 解析，SparkAPI 专用版仍由 access policy 禁止自定义 Base URL；不把 API Key 或厂商响应暴露到 Renderer，异步创建不明确不得自动重建 | `test:image-generation-adapters`, `test:image-generation-service`, `test:image-generation-async`, `test:custom-api-transport`, `test:agent-model-binding`, `test:settings-secret-store`, `typecheck`, `build` |
