...
- **Status**：partially verified。代码、mock/selftest、GUI 快速冒烟、生产 build、双版本 EXE 和远端 push 已完成；真实图片/视频服务、安装/卸载和正式发布门禁仍未运行。
- **Next**：后续若继续此 Goal，优先补 mock Agent 点击发送的 IPC 断言和安装 smoke；不得把当前证据描述为真实模型验证。
