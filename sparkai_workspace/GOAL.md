...
- **Status**：partially verified。代码、mock/selftest、真实 Electron Composer 点击、生产 build、双版本 EXE 和远端 push 已完成；真实图片/视频服务、安装/卸载和正式发布门禁仍未运行。
- **Next**：后续若继续此 Goal，优先补安装 smoke 或 provider mock 路由专项；不得把当前证据描述为真实模型验证。
